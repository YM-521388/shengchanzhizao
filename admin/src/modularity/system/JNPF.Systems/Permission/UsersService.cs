...
public class UsersService : IUsersService, IDynamicApiController, ITransient
{
    ...
    private readonly ISqlSugarRepository<OrganizeEntity> _repositoryOrganize;  // 用户表仓储
    ...
    private string cacheKeyToken = "APPUser_API_token_";
    //密钥
    ...
    /// </summary>
    public UsersService(
        ISqlSugarRepository<OrganizeEntity> repositoryOrganize,
        ...
        IMHandler imHandler)
    {
        _repositoryOrganize = repositoryOrganize;
    ...
    {
        var excelData = await GetImportDataList(fileName);
    ...
    public async Task<dynamic> ImportData([FromBody] UserImportDataInput list)
    {
        if (list.type)
        {
            var excelData = await GetImportDataList(list.fileName);
            list.list = excelData.ToObject<List<UserListImportDataInput>>();
        }
    ...
    /// <returns></returns>
    private async Task<DataTable> GetImportDataList(string fileName)
    {
    ...
    /// 登录（大屏）
    /// </summary>
    /// <param name="input">password需要传md5加密后的数据</param>
    /// <returns></returns>
    [HttpPost("H5Login")]
    public async Task<dynamic> H5Login([FromBody] UserCrInput input)
    {
        var userEntity = await _repository.GetFirstAsync(it => it.Account == input.account && it.DeleteMark == null && it.EnabledMark == 1);
        if (userEntity == null)
        {
            var jsonData = new {
                code = 202,
        ...
        if (userEntity.Account != "DPSystem")
        {
            var jsonData = new {
                code = 202,
                info = "请使用大屏管理员账号登录！"
        ...
        string? password = MD5Encryption.Encrypt(input.password + userEntity.Secretkey);
        if (userEntity.Password == password)
        {
            //var strToken = CreateToken(userEntity);

            var orgParent = getOrganizeParent(userEntity.OrganizeId);
            var orgEntity = _repositoryOrganize.GetFirst(it => it.DeleteMark == null && it.Id == orgParent);
            if (orgEntity != null)
            {
                userEntity.OrganizeName = orgEntity.FullName ?? "";
                userEntity.OrganizeId = orgEntity.Id ?? "";
            }
    ...
    /// <returns></returns>
    [HttpPost("APPLogin")]
    public async Task<dynamic> APPLogin([FromBody] UserCrInput input)
    {
        var userEntity = await _repository.GetFirstAsync(it => it.Account == input.account && it.DeleteMark == null && it.EnabledMark == 1);
        if (userEntity == null)
        {
    ...
    public string CreateToken(UserEntity entity)
    {
        try
        {
            long timestampSeconds = DateTime.UtcNow.Ticks;
            var cacheKey = cacheKeyToken + timestampSeconds;
            List<string> lst = new List<string> { cacheKey, entity.Account };
            lst.Sort();
            string strJoin = string.Join("", lst);
            string token = DESEncryption.Encrypt(strJoin, key);
            // 将token保存
            _cacheManager.Set(token, entity, new TimeSpan(7, 0, 0, 0));
            return token;
        }
        catch (Exception)
        {
    ...
    /// </summary>
    /// <returns></returns>
    public string getOrganizeParent(string id)
    {
        string parentId = "";
        string currentId = id;
        while (true)
        {
            // 查询当前节点的信息（包括ParentId和f_category）
            var result = _repositoryOrganize.AsQueryable()
                .Where(t => t.Id == currentId && t.DeleteMark == null)
                .Select(t => new { t.Id, t.ParentId, t.Category }) // 同时查询ParentId和f_category
                .First();
            if (result == null)
            {
                // 如果找不到记录，使用当前ID作为parentId
                parentId = currentId;
                break;
            }
            // 如果当前节点是公司节点，使用其ID作为parentId
