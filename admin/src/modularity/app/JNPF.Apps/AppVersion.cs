...
public class AppVersion : IDynamicApiController, ITransient
{
    ...
    public async Task<dynamic> GetInfo()
    {
        ...
        SysConfigEntity? dataLink = new SysConfigEntity();
        ...
        else
        {
            ...
            dataLink = await _repository.AsQueryable().Where(x => x.Category.Equals("SysConfig") && x.Key == "downloadLink").FirstAsync();
        }

        return new { sysVersion = data.Value, downloadLink = dataLink.Value };
    }
    ...
    /// <returns></returns>
    [HttpGet("EmployeeIp")]
    public async Task<dynamic> GetEmployeeIp()
    {
        SysConfigEntity? data = new SysConfigEntity();

        data = await _repository.AsQueryable().Where(x => x.Category.Equals("SysConfig") && x.Key == "employeeIp").FirstAsync();

        return new { employeeIp = data.Value };
    }
    #endregion
}