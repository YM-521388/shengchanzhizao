...
/// 业务实现：机台类别.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "AProdCategory", Order = 200)]
[Route("api/example/[controller]")]
public class AProdCategoryService : IAProdCategoryService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<AProdCategoryEntity> _repository;
    ...
    /// <summary>
    /// 初始化一个<see cref="AProdCategoryService"/>类型的新实例.
    /// </summary>
    public AProdCategoryService(
        ISqlSugarRepository<AProdCategoryEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new AProdCategoryEntity
            {
                id = it.id,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<AProdCategoryInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] AProdCategoryListQueryInput input)
    {
        ...
        {
            authorizeWhere = await _userManager.GetConditionAsync<AProdCategoryListOutput>(input.menuId, "F_Id", _userManager.UserOrigin.Equals("pc") ? true : false, 1);
        }

        var entityInfo = _repository.AsSugarClient().EntityMaintenance.GetEntityInfo<AProdCategoryEntity>();
        ...
        var data = await _repository.AsQueryable()
            ...
            .Select(it => new AProdCategoryListOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
        ...
        await _repository.AsSugarClient().ThenMapperAsync(data.list, async item =>
        {
            ...
            {
                var F_ParentIdData = await _dataInterfaceService.GetDynamicList("F_ParentId", "2011621811959238656", "F_Id", "F_Name", "");
                item.F_ParentId = F_ParentIdData.Find(it => it.id.Equals(item.F_ParentId))?.fullName;
            }

        });

        return PageResult<AProdCategoryListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] AProdCategoryCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AProdCategoryEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<AProdCategoryEntity>(new AProdCategoryEntity()));
    ...
    /// <param name="input">请求参数.</param>
    /// <returns></returns>
    private async Task<dynamic> GetNoPagingList(AProdCategoryListQueryInput input)
    {
        var authorizeWhere = new List<IConditionalModel>();
    ...
    public async Task<dynamic> Export([FromBody] AProdCategoryListQueryInput input)
    {
        var exportData = new List<AProdCategoryListOutput>();
        if (input.dataType == 0)
            exportData = Clay.Object(await GetList(input)).Solidify<PageResult<AProdCategoryListOutput>>().list;
        else
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] AProdCategoryUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AProdCategoryEntity>();
        var isOk =0;
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new AProdCategoryDetailOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
