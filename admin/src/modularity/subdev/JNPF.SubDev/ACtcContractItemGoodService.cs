...
/// 业务实现：项目商品用料清单.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "ACtcContractItemGood", Order = 200)]
[Route("api/example/[controller]")]
public class ACtcContractItemGoodService : IACtcContractItemGoodService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<ACtcContractItemGoodEntity> _repository;
    ...
    /// <summary>
    /// 初始化一个<see cref="ACtcContractItemGoodService"/>类型的新实例.
    /// </summary>
    public ACtcContractItemGoodService(
        ISqlSugarRepository<ACtcContractItemGoodEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .FirstAsync(it => it.id.Equals(id))).Adapt<ACtcContractItemGoodInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] ACtcContractItemGoodListQueryInput input)
    {
        ...
        {
            authorizeWhere = await _userManager.GetConditionAsync<ACtcContractItemGoodListOutput>(input.menuId, "F_Id", _userManager.UserOrigin.Equals("pc") ? true : false, 1);
        }
        ...
        var data = await _repository.AsQueryable()
            ...
            .Select(it => new ACtcContractItemGoodListOutput
            {
                id = it.id,
        ...
        return PageResult<ACtcContractItemGoodListOutput>.SqlSugarPageResult(data);
    }

    [HttpPost("NotList")]
    public async Task<dynamic> GetNotList([FromBody] ACtcContractItemGoodListQueryInput input)
    {
        var data = await _repository.AsQueryable()
            .WhereIF(!string.IsNullOrEmpty(input.F_ProjectItemId), it => it.F_ProjectItemId.Equals(input.F_ProjectItemId))
            .WhereIF(!string.IsNullOrEmpty(input.F_GoodsId), it => it.F_GoodsId.Equals(input.F_GoodsId))
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] ACtcContractItemGoodCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<ACtcContractItemGoodEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<ACtcContractItemGoodEntity>(new ACtcContractItemGoodEntity()));
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] ACtcContractItemGoodUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<ACtcContractItemGoodEntity>();
        var isOk =0;
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new ACtcContractItemGoodDetailOutput
            {
                id = it.id,
                F_ProjectItemId = it.F_ProjectItemId,
