...
/// 业务实现：报工记录日志.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "AProdReportLog", Order = 200)]
[Route("api/example/[controller]")]
public class AProdReportLogService : IAProdReportLogService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<AProdReportLogEntity> _repository;
    ...
    /// 导出字段.
    /// </summary>
    private readonly List<ParamsModel> paramList = "[{\"value\":\"报工记录\",\"field\":\"F_ReportId\"},{\"value\":\"标题\",\"field\":\"F_Title\"},{\"value\":\"内容\",\"field\":\"F_Content\"},{\"value\":\"创建人员\",\"field\":\"F_CreatorUserId\"},{\"value\":\"创建时间\",\"field\":\"F_CreatorTime\"}]".ToList<ParamsModel>();

    /// <summary>
    /// 初始化一个<see cref="AProdReportLogService"/>类型的新实例.
    /// </summary>
    public AProdReportLogService(
        ISqlSugarRepository<AProdReportLogEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new AProdReportLogEntity
            {
                id = it.id,
                F_ReportId = it.F_ReportId,
                F_Title = it.F_Title,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<AProdReportLogInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] AProdReportLogListQueryInput input)
    {
        var selectIds = input.selectIds?.Split(",").ToList();
        var data = await _repository.AsQueryable()
            .WhereIF(!string.IsNullOrEmpty(input.F_ReportId), it => it.F_ReportId == input.F_ReportId)
            .Where(it => it.FlowId==null)
            .Select(it => new AProdReportLogListOutput
            {
                id = it.id,
                F_ReportId = it.F_ReportId,
                F_Title = it.F_Title,
        ...
        return PageResult<AProdReportLogListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] AProdReportLogCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AProdReportLogEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<AProdReportLogEntity>(new AProdReportLogEntity()));
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] AProdReportLogUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AProdReportLogEntity>();
        var isOk =0;
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new AProdReportLogDetailOutput
            {
                id = it.id,
                F_ReportId = it.F_ReportId,
                F_Title = it.F_Title,
