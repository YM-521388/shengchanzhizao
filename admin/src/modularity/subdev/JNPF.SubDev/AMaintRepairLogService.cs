...
/// 业务实现：维修工单日志.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "AMaintRepairLog", Order = 200)]
[Route("api/example/[controller]")]
public class AMaintRepairLogService : IAMaintRepairLogService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<AMaintRepairLogEntity> _repository;
    ...
    /// 导出字段.
    /// </summary>
    private readonly List<ParamsModel> paramList = "[{\"value\":\"标题\",\"field\":\"F_Title\"},{\"value\":\"内容\",\"field\":\"F_Content\"},{\"value\":\"创建时间\",\"field\":\"F_CreatorTime\"}]".ToList<ParamsModel>();

    /// <summary>
    /// 初始化一个<see cref="AMaintRepairLogService"/>类型的新实例.
    /// </summary>
    public AMaintRepairLogService(
        ISqlSugarRepository<AMaintRepairLogEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new AMaintRepairLogEntity
            {
                id = it.id,
                F_RepairId = it.F_RepairId,
                F_Title = it.F_Title,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<AMaintRepairLogInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] AMaintRepairLogListQueryInput input)
    {
        var selectIds = input.selectIds?.Split(",").ToList();
        var data = await _repository.AsQueryable()
            .WhereIF(!string.IsNullOrEmpty(input.F_RepairId), it => it.F_RepairId.Contains(input.F_RepairId))
            .WhereIF(!string.IsNullOrEmpty(input.F_Title), it => it.F_Title.Contains(input.F_Title))
            .WhereIF(!string.IsNullOrEmpty(input.F_Content), it => it.F_Content.Contains(input.F_Content))
            .Where(it => it.FlowId==null)
            .Select(it => new AMaintRepairLogListOutput
            {
                id = it.id,
                F_Title = it.F_Title,
                F_Content = it.F_Content,
        ...
        return PageResult<AMaintRepairLogListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] AMaintRepairLogCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AMaintRepairLogEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<AMaintRepairLogEntity>(new AMaintRepairLogEntity()));
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] AMaintRepairLogUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AMaintRepairLogEntity>();
        var isOk =0;
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new AMaintRepairLogDetailOutput
            {
                id = it.id,
                F_RepairId = it.F_RepairId,
                F_Title = it.F_Title,
