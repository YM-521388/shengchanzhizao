...
using JNPF.example.Entitys.Dto.AWorkshopCategory;
...
/// 业务实现：车间.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "AWorkshopCategory", Order = 200)]
[Route("api/example/[controller]")]
public class AWorkshopCategoryService : IAWorkshopCategoryService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<AWorkshopCategoryEntity> _repository;
    ...
    /// 导出字段.
    /// </summary>
    private readonly List<ParamsModel> paramList = "[{\"value\":\"车间名称\",\"field\":\"F_Name\"},{\"value\":\"上级\",\"field\":\"F_ParentId\"},{\"value\":\"排序\",\"field\":\"F_SortCode\"},{\"value\":\"备注\",\"field\":\"F_Description\"}]".ToList<ParamsModel>();

    /// <summary>
    /// 初始化一个<see cref="AWorkshopCategoryService"/>类型的新实例.
    /// </summary>
    public AWorkshopCategoryService(
        ISqlSugarRepository<AWorkshopCategoryEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new AWorkshopCategoryEntity
            {
                id = it.id,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<AWorkshopCategoryInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] AWorkshopCategoryListQueryInput input)
    {
        ...
        {
            authorizeWhere = await _userManager.GetConditionAsync<AWorkshopCategoryListOutput>(input.menuId, "F_Id", _userManager.UserOrigin.Equals("pc") ? true : false, 1);
        }

        var entityInfo = _repository.AsSugarClient().EntityMaintenance.GetEntityInfo<AWorkshopCategoryEntity>();
        ...
        var data = await _repository.AsQueryable()
            ...
            .Select(it => new AWorkshopCategoryListOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
        ...
        await _repository.AsSugarClient().ThenMapperAsync(data.list, async item =>
        {
            ...
            {
                var F_ParentIdData = await _dataInterfaceService.GetDynamicList("F_ParentId", "2011621894347952128", "F_Id", "F_Name", "");
                item.F_ParentId = F_ParentIdData.Find(it => it.id.Equals(item.F_ParentId))?.fullName;
            }

        });

        return PageResult<AWorkshopCategoryListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] AWorkshopCategoryCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AWorkshopCategoryEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<AWorkshopCategoryEntity>(new AWorkshopCategoryEntity()));
    ...
    /// <param name="input">请求参数.</param>
    /// <returns></returns>
    private async Task<dynamic> GetNoPagingList(AWorkshopCategoryListQueryInput input)
    {
        var authorizeWhere = new List<IConditionalModel>();
    ...
    public async Task<dynamic> Export([FromBody] AWorkshopCategoryListQueryInput input)
    {
        var exportData = new List<AWorkshopCategoryListOutput>();
        if (input.dataType == 0)
            exportData = Clay.Object(await GetList(input)).Solidify<PageResult<AWorkshopCategoryListOutput>>().list;
        else
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] AWorkshopCategoryUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AWorkshopCategoryEntity>();
        var isOk =0;
    ...
    public async Task BatchRemove([FromBody] BatchRemoveInput input)
    {
        ...
        if (entitys.Count > 0)
        {
            var entityNum = await _repository.AsQueryable().Where(it => it.DeleteMark == null && it.F_ParentId == input.ids[0]).CountAsync();
            if (entityNum > 0)
            {
                throw Oops.Oh(ErrorCode.COM1018, "该类别存在下级，不能删除");
            }
            // 批量删除车间
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new AWorkshopCategoryDetailOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
