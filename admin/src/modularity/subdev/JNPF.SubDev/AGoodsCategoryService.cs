...
/// 业务实现：商品管理类别.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "AGoodsCategory", Order = 200)]
[Route("api/example/[controller]")]
public class AGoodsCategoryService : IAGoodsCategoryService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<AGoodsCategoryEntity> _repository;
    ...
    /// 导出字段.
    /// </summary>
    private readonly List<ParamsModel> paramList = "[{\"value\":\"分类名称\",\"field\":\"F_Name\"},{\"value\":\"分类编号\",\"field\":\"F_Code\"},{\"value\":\"上级ID\",\"field\":\"F_ParentId\"},{\"value\":\"排序\",\"field\":\"F_SortCode\"},{\"value\":\"备注\",\"field\":\"F_Description\"}]".ToList<ParamsModel>();

    /// <summary>
    /// 初始化一个<see cref="AGoodsCategoryService"/>类型的新实例.
    /// </summary>
    public AGoodsCategoryService(
        ISqlSugarRepository<AGoodsCategoryEntity> repository,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new AGoodsCategoryEntity
            {
                id = it.id,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<AGoodsCategoryInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] AGoodsCategoryListQueryInput input)
    {
        ...
        {
            authorizeWhere = await _userManager.GetConditionAsync<AGoodsCategoryListOutput>(input.menuId, "F_Id", _userManager.UserOrigin.Equals("pc") ? true : false, 1);
        }

        var entityInfo = _repository.AsSugarClient().EntityMaintenance.GetEntityInfo<AGoodsCategoryEntity>();
        ...
        var data = await _repository.AsQueryable()
            ...
            .Select(it => new AGoodsCategoryListOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
        ...
        await _repository.AsSugarClient().ThenMapperAsync(data.list, async item =>
        {
            ...
            {
                var F_ParentIdData = await _dataInterfaceService.GetDynamicList("F_ParentId", "2008421268986138624", "F_Id", "F_Name", "");
                item.F_ParentId = F_ParentIdData.Find(it => it.id.Equals(item.F_ParentId))?.fullName;
            }

        });

        return PageResult<AGoodsCategoryListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] AGoodsCategoryCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AGoodsCategoryEntity>();
        entity.id = SnowflakeIdHelper.NextId();

        var entityList = new List<AGoodsCategoryEntity>();
        if (string.IsNullOrEmpty(input.F_ParentId))
        {
            entityList = await _repository.AsQueryable().Where(it => string.IsNullOrEmpty(it.F_ParentId) && it.F_Code == input.F_Code).ToListAsync();
        }
        else
        {
            entityList = await _repository.AsQueryable().Where(it => it.F_ParentId == input.F_ParentId && it.F_Code == input.F_Code).ToListAsync();
        }
        if (entityList.Count > 0) throw Oops.Oh(ErrorCode.COM1018, "分类编号重复");
    ...
    /// <param name="input">请求参数.</param>
    /// <returns></returns>
    private async Task<dynamic> GetNoPagingList(AGoodsCategoryListQueryInput input)
    {
        var authorizeWhere = new List<IConditionalModel>();
    ...
    public async Task<dynamic> Export([FromBody] AGoodsCategoryListQueryInput input)
    {
        var exportData = new List<AGoodsCategoryListOutput>();
        if (input.dataType == 0)
            exportData = Clay.Object(await GetList(input)).Solidify<PageResult<AGoodsCategoryListOutput>>().list;
        else
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] AGoodsCategoryUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<AGoodsCategoryEntity>();
        ...
        if (entityList.Count > 0 && !entityList.Any(it => it.id == id)) throw Oops.Oh(ErrorCode.COM1018,"分类编号重复");
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new AGoodsCategoryDetailOutput
            {
                id = it.id,
                F_ParentId = it.F_ParentId,
                F_Name = it.F_Name,
