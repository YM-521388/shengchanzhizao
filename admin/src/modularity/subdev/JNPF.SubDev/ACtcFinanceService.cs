...
/// 业务实现：收支记录.
/// </summary>
[ApiDescriptionSettings(Tag = "example", Name = "ACtcFinance", Order = 200)]
[Route("api/example/[controller]")]
public class ACtcFinanceService : IACtcFinanceService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    private readonly ISqlSugarRepository<ACtcFinanceEntity> _repository;
    private readonly ISqlSugarRepository<ACtcContractEntity> _repositoryht;
    ...
    /// 导出字段.
    /// </summary>
    private readonly List<ParamsModel> paramList = "[{\"value\":\"类型(收/支)\",\"field\":\"F_Type\"},{\"value\":\"金额\",\"field\":\"F_Money\"},{\"value\":\"附件\",\"field\":\"F_Files\"},{\"value\":\"审核状态\",\"field\":\"F_AuditStatus\"},{\"value\":\"备注\",\"field\":\"F_Description\"},{\"value\":\"创建人员\",\"field\":\"F_CreatorUserId\"},{\"value\":\"创建时间\",\"field\":\"F_CreatorTime\"},{\"value\":\"合同ID\",\"field\":\"F_ContractId\"}]".ToList<ParamsModel>();

    /// <summary>
    /// 初始化一个<see cref="ACtcFinanceService"/>类型的新实例.
    /// </summary>
    public ACtcFinanceService(
        ISqlSugarRepository<ACtcFinanceEntity> repository,
         ISqlSugarRepository<ACtcContractEntity> repositoryht,
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        var data = (await _repository.AsQueryable()
            .Select(it => new ACtcFinanceEntity
            {
                id = it.id,
            ...
            .FirstAsync(it => it.id.Equals(id))).Adapt<ACtcFinanceInfoOutput>(); 
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] ACtcFinanceListQueryInput input)
    {
        ...
        {
            authorizeWhere = await _userManager.GetConditionAsync<ACtcFinanceListOutput>(input.menuId, "F_Id", _userManager.UserOrigin.Equals("pc") ? true : false, 1);
        }

        var entityInfo = _repository.AsSugarClient().EntityMaintenance.GetEntityInfo<ACtcFinanceEntity>();
        ...
        var data = await _repository.AsQueryable()
            ...
            .Select(it => new ACtcFinanceListOutput
            {
                id = it.id,
                F_ContractId = SqlFunc.Subqueryable<ACtcContractEntity>().Where(c => c.id.Equals(it.F_ContractId)).Select(c => c.F_ContractCode),
                F_Type = it.F_Type,
                F_Money = it.F_Money,
                F_Files = it.F_Files,
                F_AuditStatus = it.F_AuditStatus,
        ...
        await _repository.AsSugarClient().ThenMapperAsync(data.list, async item =>
        {
            ...
            // 类型(收/支)
            var F_TypeData = "[{\"id\":\"收入\",\"fullName\":\"收入\"},{\"id\":\"支出\",\"fullName\":\"支出\"}]".ToObject<List<StaticDataModel>>();
            if(item.F_Type != null)
            {
                item.F_Type = F_TypeData.Find(it => item.F_Type.Equals(it.id))?.fullName;
            }
        ...
        return PageResult<ACtcFinanceListOutput>.SqlSugarPageResult(data);
    }
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task Create([FromBody] ACtcFinanceCrInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<ACtcFinanceEntity>();
        entity.id = SnowflakeIdHelper.NextId();
        List<FieldsModel> fieldList = new List<FieldsModel>();
        fieldList.AddRange(ExportImportDataHelper.GetTemplateParsing<ACtcFinanceEntity>(new ACtcFinanceEntity()));
    ...
    /// <param name="input">请求参数.</param>
    /// <returns></returns>
    private async Task<dynamic> GetNoPagingList(ACtcFinanceListQueryInput input)
    {
        var authorizeWhere = new List<IConditionalModel>();
    ...
    public async Task<dynamic> Export([FromBody] ACtcFinanceListQueryInput input)
    {
        var exportData = new List<ACtcFinanceListOutput>();
        if (input.dataType == 0)
            exportData = Clay.Object(await GetList(input)).Solidify<PageResult<ACtcFinanceListOutput>>().list;
        else
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] ACtcFinanceUpInput input)
    {
        input = CodeGenHelper.SetEmptyStringNull(input);
        var entity = input.Adapt<ACtcFinanceEntity>();
        var isOk =0;
    ...
    {
        var data = await _repository.AsQueryable()
            .Select(it => new ACtcFinanceDetailOutput
            {
                id = it.id,
                F_ContractId = SqlFunc.Subqueryable<ACtcContractEntity>().Where(c => c.id.Equals(it.F_ContractId)).Select(c => c.F_ContractCode),
                F_Type = it.F_Type,
