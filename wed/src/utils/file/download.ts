...
export function downloadByUrlReport({ url, target = '_blank', fileName }: { url: string; target?: TargetContext; fileName?: string }): boolean {
  if (!url) return false;
  const globSetting = useGlobSetting();
  const isChrome = window.navigator.userAgent.toLowerCase().indexOf('chrome') > -1;
  const isSafari = window.navigator.userAgent.toLowerCase().indexOf('safari') > -1;
  ...
  if (!/https?:\/\//.test(url) && url.indexOf('data:image/png;base64') < 0) url = globSetting.reportApiUrl + url;
  if (isDevMode() && isFirefox) {
    const VITE_PROXY: any = import.meta.env.VITE_PROXY || '';
    const proxyList = import.meta.env.VITE_PROXY ? JSON.parse(VITE_PROXY) : [];
    for (const [prefix, target] of proxyList) {
      if (prefix === globSetting.reportApiUrl) {
        url = url.replace(prefix, target);
        break;
      }
    }
