...
<script lang="ts" setup>
  ...
  const checkName = (_rule, value) => {
    let boo = true;
    const regex = /(^([A-Za-z]?)*$)|(^[A-Za-z](_?[A-Za-z0-9])*_?$)/;
    if (!regex.test(value)) return Promise.reject('只能以字母开头且仅包含字母、数字、下划线');
  ...
  const schemas: any[] = [
    ...
    },
    {
      ...
      component: 'Input',
      componentProps: { placeholder: '请输入' },
      dynamicRules: ({ values }) => (values.required == 0 ? [] : [{ required: true, trigger: 'blur', message: '必填' }]),
    },
    {
  ...
  const [registerForm, { setFieldsValue, resetFields, clearValidate, validate, updateSchema }] = useForm({
    labelWidth: 80,
    schemas: props.type === 0 ? schemas : schemas_,
  });
  ...
  function onRequiredChange(e) {
    nextTick(() => clearValidate('defaultValue'));
  }
