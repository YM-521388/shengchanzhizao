<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="授权用户" :width="800" @ok="handleSubmit" destroyOnClose class="transfer-modal">
...
<script lang="ts" setup>
  ...
  import { saveUserList, getInfo } from '@/api/systemData/interfaceOauth';
  ...
  function init(data) {
    ...
    state.nodeId = '0';
    state.activeKey = '1';
    getInfo(state.dataForm.objectId).then(res => {
      state.dataForm.userIds = res.data.userList.map(o => o.userId) || [];
  ...
  function handleSubmit() {
    ...
    let query = {
      interfaceIdentId: state.dataForm.objectId,
      userIds: state.dataForm.userIds,
    };
    saveUserList(query)
      .then(res => {
