...
<script lang="ts" setup>
  ...
  const schemas: FormSchema[] = [
    ...
    {
      ifShow: ({ values }) => [3, 4, 5, 6, 8, 9, 10].indexOf(values.type) > -1,
      field: 'moduleId',
      label: '关联',
      component: 'TreeSelect',
      slot: 'moduleId',
  ...
  async function handleSubmit() {
    ...
    if (state.category === 'Web') {
      ...
      if (values.type == 5) values.urlAddress = `dataReport/${menuEnCode}`; // 报表(原)
      if (values.type == 10) values.urlAddress = `report/${menuEnCode}`; // 报表
      ...
      if (values.type == 5 || values.type == 10 || values.type == 8) values.urlAddress = menuEnCode;
    }
    const query = {
      ...state.dataForm,
      ...values,
  ...
  async function fetchDataReportList() {
    if (!state.dataReportData.length) {
      ...
      getDataReportSelector().then(res => {
        let dataReportData = res.data.list;
        state.dataReportData = ReportSortTypeList;
        nextTick(() => {
          for (let i = 0; i < state.dataReportData.length; i++) {
            let child = dataReportData.filter(o => state.dataReportData[i].id === o.categoryId);
