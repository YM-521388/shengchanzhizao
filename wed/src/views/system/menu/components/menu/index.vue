<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup">
    ...
    <a-tabs v-model:activeKey="listQuery.category" class="jnpf-content-wrapper-tabs" destroyInactiveTabPane>
...
<script lang="ts" setup>
  ...
  function getDropDownActions(record): ActionItem[] {
    ...
    return [
      {
        label: '按钮权限',
        ifShow: record.isButtonAuthorize === 1 && [2, 3, 4, 9].includes(record.type),
        onClick: handleButtonAuthorize.bind(null, record),
      },
      {
        label: '列表权限',
        ifShow: record.isColumnAuthorize === 1 && [2, 3, 4, 9].includes(record.type),
        onClick: handleColumnAuthorize.bind(null, record),
      },
      {
        label: '表单权限',
        ifShow: record.isFormAuthorize === 1 && [2, 3, 4].includes(record.type),
        onClick: handleFormAuthorize.bind(null, record),
      },
      {
        label: '数据权限',
        ifShow: record.isDataAuthorize === 1 && [2, 3, 4, 9].includes(record.type),
      ...
      {
        label: '查看权限',
        ifShow: [2, 3, 4, 9].includes(record.type),
