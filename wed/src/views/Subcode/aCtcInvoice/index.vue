        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_Status'">
                <a-tag :color="record.F_StatusInfo == '1' ? 'success' : 'default'">
                  {{ record.F_Status }}
                </a-tag>
              </template>
              <!-- <template v-if="column.jnpfKey === 'input'">
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: t('common.invoiceText', '开票'),
        onClick: invoiceHandle.bind(null, record),
        // 如果已经是“开票完成”则禁用开票按钮
        disabled: record.F_Status === '开票完成',
      },
    ];
  }
  // 新增
  ...
  // 开票（始终显示，不受权限控制）
  function invoiceHandle(record) {
    const data = {
      id: record.id,
      menuId: searchInfo.menuId,
      openedFromInvoice: true,
      allList: state.cacheList,
    };
    formRef.value?.init(data);
  }
