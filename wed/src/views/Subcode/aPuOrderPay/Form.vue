    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_PayDate')">
            <a-form-item name="F_PayDate" :labelCol="{ style: { width: '100px' } }">
              <template #label>付款日期</template>
              <JnpfDatePicker
                v-model:value="dataForm.F_PayDate"
                placeholder="请选择"
                format="yyyy-MM-dd"
                allowClear
                :style="{
...
<script lang="ts" setup>
  ...
  // 接收父组件是否忽略权限检查
  const props = defineProps<{ ignorePermissions?: boolean }>();
  ...
  const { t } = useI18n();
  const { hasFormP: _hasFormP } = usePermission();
  // 包装 hasFormP，使得在传入 props.ignorePermissions 时总是返回 true，从而显示所有字段
  const hasFormP = key => (props.ignorePermissions ? true : _hasFormP(key));
  ...
  const state = reactive<State>({
    dataForm: {
      ...
      F_Money: undefined,
      F_PayDate: dayjs(new Date()).startOf(getDateTimeUnit('yyyy-MM-dd')).valueOf(),
      F_Files: [],
  ...
  function init(data) {
    ...
    // 接收预填订单 id
    state.prefillOrderId = data?.F_OrderId;
    openModal();
  ...
  function initData() {
    ...
    if (state.dataForm.id) {
      ...
      if (state.prefillOrderId) {
        state.dataForm.F_OrderId = state.prefillOrderId;
        state.prefillOrderId = undefined;
      }
