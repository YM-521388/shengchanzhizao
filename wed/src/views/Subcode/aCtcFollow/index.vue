          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_State'">
                <a-tag :color="record.F_StateCode == 1 ? 'success' : record.F_StateCode == 2 ? 'error' : 'default'">
                  {{ record.F_State }}
                </a-tag>
              </template>
            </template>
...
<script lang="ts" setup>
  ...
  function handleState(id, state) {
    createConfirm({
      iconType: 'warning',
      title: t('common.sureText', '确定已完成吗？'),
      onOk: () => {
