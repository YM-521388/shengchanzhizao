<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_TaskStatus'">
                <a-tag
                  :color="
                    record.F_TaskStatusCode == 2
                      ? 'success'
                      : record.F_TaskStatusCode == 1
                      ? 'warning'
                      : record.F_TaskStatusCode == 3
                      ? 'pink'
                      : record.F_TaskStatusCode == 4
                      ? 'error'
                      : 'default'
                  ">
                  {{ record.F_TaskStatus }}
              ...
              <template v-if="column.key === 'F_ProcessingState'">
                <a-tag
                  :color="
                    record.F_ProcessingStateCode == 2
                      ? 'success'
                      : record.F_ProcessingStateCode == 1
                      ? 'warning'
                      : record.F_ProcessingStateCode == 3
                      ? 'pink'
                      : record.F_ProcessingStateCode == 4
                      ? 'error'
                      : 'default'
                  ">
                  {{ record.F_ProcessingState }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_SettleStatus'">
                <a-tag :color="record.F_SettleStatusCode == '1' ? 'success' : 'default'">
                  {{ record.F_SettleStatus }}
                </a-tag>
              </template>
    ...
    <SettleForm ref="settleRef" @reload="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    return [
      {
        label: t('结算'),
        onClick: settleHandle.bind(null, record),
        disabled: record.F_SettleStatusCode == '1',
        auth: 'btn_settle',
      },
      {
  ...
  // 结算
  function settleHandle(record) {
    // 不带工作流
    const data = {
      id: record.id,
      F_ProcessName: record.F_ProcessName,
    };
    settleRef.value?.init(data);
  }
  // 新增
  function addHandle() {
    const data = {
