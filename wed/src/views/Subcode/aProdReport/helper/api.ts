...
enum Api {
  Prefix = '/api/example/AProdReport',
}

// 获取不良品项列表
export function getDefectList(data) {
  return defHttp.post({ url: Api.Prefix + `/DefectList`, data });
}

// 结算
export function SettleUpdate(data) {
  return defHttp.put({ url: Api.Prefix + `/SettleUpdate/` + data.id, data });
}
