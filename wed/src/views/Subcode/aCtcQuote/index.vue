<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable
                ...
                <BasicTable @register="registerChildTable" :ellipsis="true" :data-source="record[child.prop]" :columns="child.children">
                  <template #bodyCell="{ column, record: childRecord }">
                    ...
                    <template v-if="column.jnpfKey === 'input'">
                        <jnpf-input v-model:value="childRecord[column.dataIndex]" :useMask="column.useMask" :maskConfig="column.maskConfig" :showOverflow="true" detailed />
                    </template>
                  </template>
                </BasicTable>
              </a-tab-pane>
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_QuoteStatus'">
                <a-tag :color="record.F_QuoteStatusNo == 1 ? 'success' : 'warning'">
                  {{ record.F_QuoteStatus }}
                </a-tag>
              </template>
            </template>
    ...
    <Formht ref="formHtRef" @reload="reload" />
...
<script lang="ts" setup>
  import { getList, del, exportData, batchDelete, getInfo } from './helper/api';
  ...
  function getTableActions(record): ActionItem[] {
    return [
      {
        label: t('生成合同'),
        disabled: record.F_QuoteStatusNo == '1',
        onClick: generateContractHandle.bind(null, record),
        auth: 'btn_generate',
      },
      {
  ...
  // 生成合同
  async function generateContractHandle(record) {
    try {
      // 尝试获取完整的报价详情（包含子表）再打开合同新增弹窗
      const res = await getInfo(record.id);
      const quote = res && res.data ? res.data : res;
      const data = {
        id: '',
        menuId: searchInfo.menuId,
        allList: state.cacheList,
        quoteRecord: quote, // 传递完整报价记录用于初始化合同数据
      };
      formHtRef.value?.init(data);
    } catch (e) {
      // 回退：若获取失败仍传原始 record
      const data = {
        id: '',
  ...
  function getColumnList() {
    ...
    let columns = columnList.map(o => ({
      ...
      sorter: o.sortable ? { multiple: 1 } : o.sorter,
      width: o.width || 100,
      resizable: true,
    }));
  ...
  function handleResizeColumn(w, col) {
    try {
      // 直接修改传入列对象的 width（BasicTable 传入的是 columns 内部对象）
      if (col) col.width = w;
      // 触发 state.columns 响应式刷新（某些场景需要重新赋值数组）
