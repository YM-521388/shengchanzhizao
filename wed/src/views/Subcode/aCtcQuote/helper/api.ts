...
export function updateState(data) {
  return defHttp.put({ url: Api.Prefix + `/State/` + data });
}
...
// 根据客户ID获取默认联系人和地址
export function getDefaultsByCustomer(customerId) {
  return defHttp.get({ url: Api.Prefix + `/Defaults/Customer/` + customerId });
}
