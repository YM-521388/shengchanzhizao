    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          ...
          <a-col :span="12" class="ant-col-item" v-if="hasFormP('F_IsFollow')">
            <a-form-item name="F_IsFollow" :labelCol="{ style: { width: '100px' } }">
              <template #label>是否关注</template>
              <JnpfSwitch v-model:value="dataForm.F_IsFollow" />
            </a-form-item>
          </a-col>
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record, index }">
                  ...
                  <template v-if="column.key === 'F_IsDefault'">
                    <JnpfSwitch v-model:value="record.F_IsDefault" @change="val => onTableField6eed25IsDefaultChange(val, record, index)" />
                  </template>
            ...
            <a-form-item label="">
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record, index }">
                  ...
                  <template v-if="column.key === 'F_IsDefault'">
                    <JnpfSwitch v-model:value="record.F_IsDefault" @change="val => onTableFieldddababIsDefaultChange(val, record, index)" />
                  </template>
                  <template v-if="column.key === 'F_CreatorTime'">
                    <JnpfOpenData
                      v-model:value="record.F_CreatorTime"
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    optionsObj: {
      F_CustomerTagsProps: { label: 'fullName', value: 'enCode' },
    },
    currVmodel: '',
    currTableConf: {},
    tableRows: {
  ...
  function getData(id) {
    getInfo(id).then(res => {
      ...
      // 归一化：保证每个子表至多一个默认项，且至少有一条默认（若存在记录）
      if (state.dataForm.tableField6eed25 && state.dataForm.tableField6eed25.length) {
        const defaultItems = state.dataForm.tableField6eed25.filter((r: any) => r && Number(r.F_IsDefault) === 1);
        if (defaultItems.length === 0) {
          state.dataForm.tableField6eed25[0].F_IsDefault = 1;
        } else if (defaultItems.length > 1) {
          let found = false;
          state.dataForm.tableField6eed25.forEach((r: any) => {
            if (Number(r.F_IsDefault) === 1 && !found) {
              found = true;
            } else {
              r.F_IsDefault = 0;
            }
      ...
      if (state.dataForm.tableFieldddabab && state.dataForm.tableFieldddabab.length) {
        const defaultItems = state.dataForm.tableFieldddabab.filter((r: any) => r && Number(r.F_IsDefault) === 1);
        if (defaultItems.length === 0) {
          state.dataForm.tableFieldddabab[0].F_IsDefault = 1;
        } else if (defaultItems.length > 1) {
          let found = false;
          state.dataForm.tableFieldddabab.forEach((r: any) => {
            if (Number(r.F_IsDefault) === 1 && !found) {
              found = true;
            } else {
              r.F_IsDefault = 0;
  ...
  // 在名称输入完成（失焦）时调用后端校验是否重名
  async function checkDuplicateName() {
    const name = state.dataForm.F_CustomerName;
    if (!name) return;
    try {
      const res: any = await defHttp.post({ url: '/api/example/ACustomer/CheckDuplicateName', data: { F_CustomerName: name } });
      // 后端通过返回体中的 data.exists 表示是否重复（接口返回 code:200 时也可能包含重复提示）
      if (res && res.data && res.data.exists) {
        const messageText = res.data.message || '该姓名已重复';
        createMessage.error(messageText);
        // 禁用确认按钮并隐藏（双重保险，确保不可提交）
        setFormProps({ okButtonProps: { disabled: true }, showOkBtn: false });
      } else {
        // 校验通过：确保确认按钮可用并显示
        setFormProps({ okButtonProps: { disabled: false }, showOkBtn: true });
      }
    } catch (err) {
      const msg = (err && (err.msg || err.message || (err.data && err.data.message))) || '该姓名已重复';
      createMessage.error(msg);
      // 网络或服务异常时也禁用确认按钮以避免错误提交
      setFormProps({ okButtonProps: { disabled: true } });
    }
  }
  // 当用户在名称输入框输入时，恢复确认按钮（避免因为之前校验禁用导致无法提交）
  function handleCustomerNameInput(_: any) {
    // 姓名为空或修改时均允许点击确定，后续以失焦校验为准
    setFormProps({ okButtonProps: { disabled: false }, showOkBtn: true });
  }
  ...
  // 已移除：自动将地区文本写入地址字段的逻辑（按要求取消）
  // 保证子表“客户地址”只有一条默认记录
  function onTableField6eed25IsDefaultChange(val: any, record: any, index: number) {
    if (val) {
      (state.dataForm.tableField6eed25 || []).forEach((r: any) => {
  ...
  // 保证子表“客户联系人”只有一条默认记录
  function onTableFieldddababIsDefaultChange(val: any, record: any, index: number) {
    if (val) {
      (state.dataForm.tableFieldddabab || []).forEach((r: any) => {
        if (r && r.jnpfId !== record.jnpfId) r.F_IsDefault = 0;
      });
  ...
  function addtableField6eed25Row() {
    const isFirst = !(state.dataForm.tableField6eed25 && state.dataForm.tableField6eed25.length);
    let item = {
      jnpfId: buildUUID(),
      F_Region: [],
      F_Address: undefined,
    ...
    if (state.dataForm.tableField6eed25.length === 1) state.dataForm.tableField6eed25[0].F_IsDefault = 1;
  ...
  function removetableField6eed25Row(index, showConfirm = false) {
    if (showConfirm) {
      createConfirm({
        ...
        onOk: () => {
          const removed = state.dataForm.tableField6eed25.splice(index, 1)[0];
          // 若删除的是默认项，删除后确保仍有一个默认（若有剩余记录则把第一条设为默认）
          if (removed && Number(removed.F_IsDefault) === 1) {
            if (state.dataForm.tableField6eed25 && state.dataForm.tableField6eed25.length) {
              state.dataForm.tableField6eed25[0].F_IsDefault = 1;
            }
          }
  ...
  function batchRemovetableField6eed25Row(showConfirm = false) {
    ...
    const handleBatchRemove = () => {
      const remaining = state.dataForm.tableField6eed25.filter(o => !state.selectedtableField6eed25RowKeys.includes(o.jnpfId));
      state.dataForm.tableField6eed25 = remaining;
      nextTick(() => {
        state.selectedtableField6eed25RowKeys = [];
      });
      // 确保至少有一条默认项
      if (state.dataForm.tableField6eed25 && state.dataForm.tableField6eed25.length) {
        const hasDefault = state.dataForm.tableField6eed25.some((r: any) => Number(r.F_IsDefault) === 1);
        if (!hasDefault) state.dataForm.tableField6eed25[0].F_IsDefault = 1;
      }
    };
  ...
  function addtableFieldddababRow() {
    const isFirst = !(state.dataForm.tableFieldddabab && state.dataForm.tableFieldddabab.length);
    let item = {
      jnpfId: buildUUID(),
      F_ContactName: undefined,
      F_ContactPhone: undefined,
    ...
    if (state.dataForm.tableFieldddabab.length === 1) state.dataForm.tableFieldddabab[0].F_IsDefault = 1;
  ...
  function removetableFieldddababRow(index, showConfirm = false) {
    if (showConfirm) {
      createConfirm({
        ...
        content: '此操作将永久删除该数据, 是否继续?',
        onOk: () => {
          const removed = state.dataForm.tableFieldddabab.splice(index, 1)[0];
          if (removed && Number(removed.F_IsDefault) === 1) {
            if (state.dataForm.tableFieldddabab && state.dataForm.tableFieldddabab.length) {
              state.dataForm.tableFieldddabab[0].F_IsDefault = 1;
            }
  ...
  function batchRemovetableFieldddababRow(showConfirm = false) {
    ...
    const handleBatchRemove = () => {
      const remaining = state.dataForm.tableFieldddabab.filter(o => !state.selectedtableFieldddababRowKeys.includes(o.jnpfId));
      state.dataForm.tableFieldddabab = remaining;
      nextTick(() => {
        state.selectedtableFieldddababRowKeys = [];
      });
      if (state.dataForm.tableFieldddabab && state.dataForm.tableFieldddabab.length) {
        const hasDefault = state.dataForm.tableFieldddabab.some((r: any) => Number(r.F_IsDefault) === 1);
        if (!hasDefault) state.dataForm.tableFieldddabab[0].F_IsDefault = 1;
      }
    };
    if (!showConfirm) return handleBatchRemove();
    createConfirm({
  ...
  function addForSelect(data) {
    ...
    // 如果当前表在添加后只有一条数据，确保这条为默认
    if (state.dataForm[state.currVmodel] && state.dataForm[state.currVmodel].length === 1) {
      state.dataForm[state.currVmodel][0].F_IsDefault = 1;
    }
  }
