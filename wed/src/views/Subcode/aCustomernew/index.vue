          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_State'">
                <a-tag :color="record[column.dataIndex] == '开' ? 'success' : 'error'">
                  {{ record[column.dataIndex] == '开' ? '启用' : '禁用' }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_IsFollow'">
                <a-tag :color="record[column.dataIndex] == '开' ? 'success' : 'error'">
                  {{ record[column.dataIndex] == '开' ? '是' : '否' }}
                </a-tag>
              </template>
              <template v-if="column.jnpfKey === 'input'">
            ...
            <template v-if="column.key === 'action' && !record.top">
              <div class="action-row">
                ...
                <!-- <div class="action-item">
                  <a-dropdown>
                    <a-button type="link" class="action-button">
                      {{ t('common.moreText', '更多') }}
                      <MoreOutlined />
                    </a-button>
                    <template #overlay>
                      <a-menu @click="() => moveToPublicSea(record)">
                        <a-menu-item key="move" class="menu-move">移入公海</a-menu-item>
                      </a-menu>
                    </template>
                  </a-dropdown>
                </div> -->
...
<script lang="ts" setup>
  import { getList, del, exportData, batchDelete, update, moveToPublicSea as moveToPublicSeaApi } from './helper/api';
  ...
  import { FilterOutlined, MoreOutlined } from '@ant-design/icons-vue';
  ...
  function getDropDownActions(record): ActionItem[] {
    return [
      {
        label: '移入公海',
        onClick: moveToPublicSea.bind(null, record),
        auth: 'btn_moveSeas',
      },
    ];
  }
  ...
  // 移入公海
  function moveToPublicSea(record) {
    createConfirm({
      iconType: 'warning',
      title: '提示',
      content: '确定将该客户移入公海吗？',
      onOk: () => {
        createMessage.info('正在移入公海...');
        moveToPublicSeaApi(record.id)
          .then(res => {
            createMessage.success(res.msg || '移入公海成功');
            reload();
          })
          .catch(() => {
