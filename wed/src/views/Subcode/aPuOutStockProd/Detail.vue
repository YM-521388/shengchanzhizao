    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" ref="formRef">
        <a-row :gutter="15">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('SCCK_WarehouseId')">
          ...
          <!-- <a-col :span="8" class="ant-col-item" v-if="hasFormP('SCCK_Method')">
...
<script lang="ts" setup>
  ...
  const allColumns: BasicColumn[] = [
    ...
    { title: '库存', dataIndex: 'F_InventoryNum', resizable: true, width: 100 },
    { title: '出库数量', dataIndex: 'F_NumInfo', resizable: true, width: 100 },
  ...
  const columns = computed(() => allColumns.filter(col => typeof col.dataIndex === 'string' && hasFormP(('tableField2752cf-' + col.dataIndex) as string)));
  const [registerTable, { reload }] = useTable({
    api: getGoodsList,
    beforeFetch: data => {
      data.F_StockOutProdId = state.dataForm.id;
      return data;
    },
    columns,
    useSearchForm: false,
  ...
  const columnsLog = computed(() =>
    allColumnsLog.filter(col => typeof col.dataIndex === 'string' && hasFormP(('tableField49228c-' + col.dataIndex) as string)),
  );
