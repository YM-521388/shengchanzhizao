      ...
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="24" class="ant-col-item">
            <a-form-item name="F_ParentId" :labelCol="{ style: { width: '100px' } }">
              <template #label>上级</template>
              <JnpfCascader
                v-model:value="dataForm.F_ParentId"
                placeholder="请选择"
                :options="optionsObj.F_ParentIdOptions"
                :fieldNames="optionsObj.F_ParentIdProps"
                allowClear
                :disabled="isEdit"
                :style="{
                  width: '100%',
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Name')">
            <a-form-item name="F_Name" :labelCol="{ style: { width: '100px' } }">
              <template #label>名称</template>
              <JnpfInput
                v-model:value="dataForm.F_Name"
                placeholder="请输入"
                allowClear
                :style="{
                  width: '100%',
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Address')">
            <a-form-item name="F_Address" :labelCol="{ style: { width: '100px' } }">
              <template #label>编码</template>
              <JnpfInput
                v-model:value="dataForm.F_Address"
                placeholder="为空时，系统自动生成"
                allowClear
                :style="{
                  width: '100%',
                }"
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_UsersId')">
            <a-form-item name="F_UsersId" :labelCol="{ style: { width: '100px' } }">
              <template #label>管理员</template>
              <JnpfUserSelect
                v-model:value="dataForm.F_UsersId"
                placeholder="请选择"
                selectType="all"
                multiple
                allowClear
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Location')">
            <a-form-item name="F_Location" :labelCol="{ style: { width: '100px' } }">
              <template #label>仓库位置</template>
              <JnpfAreaSelect
                v-model:value="dataForm.F_Location"
                placeholder="请选择"
                allowClear
                :level="2"
                :style="{
          ...
          <a-col :span="24" class="ant-col-item" v-show="false" v-if="hasFormP('F_QRCode')">
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['reload', 'reloadTree']);
  ...
  const isEdit = computed(() => !!state.dataForm.id);
  ...
  function getF_ParentIdOptions() {
    ...
    getDataInterfaceRes('2021045201115680768', query).then(res => {
      let data = res.data;
      state.optionsObj.F_ParentIdOptions = Array.isArray(data) ? data : [];
    });
  }
