<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <!-- 状态字段：值为 "开" 显示绿色（启用），否则显示红色（禁用） -->
              <template v-if="column.key === 'F_State'">
                <a-tag :color="record[column.dataIndex] == '启用' ? 'success' : 'error'">
                  {{ record[column.dataIndex] == '启用' ? '启用' : '禁用' }}
                </a-tag>
              </template>
    ...
    <Form ref="formRef" @reload="reload" @reloadTree="getTreeView()" />
...
<script lang="ts" setup>
  ...
  function handleLeftTreeSelect(id, _node, nodePath) {
    ...
    queryJson = { F_ParentId: currValue };
    leftTreeActiveInfo = { F_ParentId: state.treeRelationObj?.multiple ? [currValue] : currValue };
  ...
  function getSearchSchemas() {
    // 有左侧树，有关联字段
    const superQueryJson = searchInfo.superQueryJson ? JSON.parse(searchInfo.superQueryJson) : [];
  ...
  function getColumnList() {
    ...
    // // 强制显示二维码 F_QRCode 字段，不受权限控制
    // const forceShowFields = ['F_QRCode'];
    // for (let i = 0; i < forceShowFields.length; i++) {
    //   const fieldName = forceShowFields[i];
    //   const exists = columnList.some(o => o.prop === fieldName);
    //   if (!exists) {
    //     const target = state.columnList.find(o => o.prop === fieldName);
    //     if (target) {
    //       columnList.push(target);
    //     }
    //   }
    // }

    // 取消权限控制，显示所有字段
    let columnList: any[] = [...state.columnList];
  ...
  async function downloadQrCode(record) {
    try {
      if (!record.F_Address) {
        createMessage.warning('地址为空,无法生成条形码');
        return;
      }
      // 创建canvas生成条形码
      const canvas = document.createElement('canvas');
      JsBarcode(canvas, record.F_Address, {
        format: 'CODE128',
        width: 2,
        height: 80,
        displayValue: true,
      ...
      downloadByData(blob, `${record.F_Name}.png`, mime);
    ...
    try {
      ...
      for (let i = 0; i < dataToDownload.length; i++) {
        ...
        try {
          ...
          zip.file(`${record.F_Name}.png`, base64Data, { base64: true });
        } catch (error) {
          console.error(`生成 ${record.F_Name} 条形码失败:`, error);
        }
      }

      const content = await zip.generateAsync({ type: 'blob' });
      downloadByData(content, '仓库条形码.zip', 'application/zip');
      createMessage.success('下载成功');
    } catch (error) {
      createMessage.error('下载失败');
      console.error(error);
