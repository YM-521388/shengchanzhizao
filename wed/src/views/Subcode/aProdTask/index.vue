        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_TaskType'">
                <a-tag :color="record.F_TaskTypeCode == 1 ? 'success' : 'warning'">
                  {{ record.F_TaskType }}
                </a-tag>
              </template>
            </template>
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      },
      {
        ...
        onClick: reportHandle.bind(null, record),
        auth: 'btn_report',
      },
    ];
  } //把按钮放在更多中
  ...
  function reportHandle(record) {
    const data = {
      ...
      F_ProdUserByIds: record.F_ProdUserByIds,
