    ...
    <div class="content-container">
      <div class="top-wrapper">
        ...
        <div class="width-35-container num-container">
          <div class="grid-item" :style="{ 'background-color': item.bgColor }" v-for="(item, index) in state.numData" :key="index">
            <div>{{ item.name }}</div>
            <div>
      ...
      <div class="bottom-wrapper">
        ...
        <div class="content-common-container">
          ...
          <div class="table-content-container">
            <ScrollList :data="state.workOrderData" ref="workOrderRef" class="work-order-container">
              <template #item="{ item }">
                <div class="table-item">
                  ...
                  <div class="table-w-40">
                    <div class="process-flow-container">
                      <template v-for="(task, indexs) in item.ProdTaskList" :key="task.id">
                        <div class="process-flow-item">
                          <!-- 进度环 - SVG实现 -->
                          <div class="progress-circle-wrapper">
                            <svg class="progress-svg" viewBox="0 0 44 44">
                              <!-- 背景圆环 -->
                              <circle :class="task.F_CompletedQty == '0' ? 'progress-track-0' : 'progress-track'" cx="22" cy="22" r="18" />
                              <!-- 进度圆环 占比-->
                              <circle
                                class="progress-bar"
                                cx="22"
                                ...
                                :stroke-dashoffset="getCircleOffset(task)"
                        ...
                        <!-- 连接线 -->
                        <div class="process-connector" v-if="indexs < item.ProdTaskList.length - 1"></div>
                      </template>
                    </div>
                  </div>
                </div>
...
<script setup lang="ts">
  ...
  const getCircleOffset = task => {
    ...
    const circumference = 2 * Math.PI * 18;
    const offset = circumference * (1 - percent / 100);
    return percent > 0 ? 0 : 113.09733552923255;
  };
