...
<script lang="ts" setup>
  ...
  import FinanceForm from '../aCtcFinance/Form.vue';
  import InvoiceForm from '../aCtcInvoice/Form.vue';
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: '收款',
        onClick: receiveHandle.bind(null, record),
        auth: 'btn_payment',
      },
      {
        label: '支出',
        onClick: payHandle.bind(null, record),
        auth: 'btn_expend',
      },
      {
        label: '开发票',
        onClick: issueInvoice.bind(null, record),
        auth: 'btn_invoice',
      },
      {
  ...
  // 开发票（占位实现，可替换为实际弹窗或接口调用）
  function issueInvoice(record) {
    // 将当前合同 id 写入左侧树状态，供发票表单注入读取
    state.leftTreeActiveInfo = { F_ContractId: record.id };
    const data = {
      id: '', // 新增开票
      allList: state.cacheList,
      // 标记来自合同列表，以便目标表单根据来源禁用合同字段
      openedFromListEdit: true,
      openedFromContractList: true,
      F_ContractId: record.id,
    };
    invoiceRef.value?.init(data);
  }
  // 新增
  function addHandle() {
    const data = {
  ...
  // 收款：打开收支记录新增，默认写入合同ID并设为“收入”
  function receiveHandle(record) {
    state.leftTreeActiveInfo = { F_ContractId: record.id, F_Type: '收入' };
    const data = {
      id: '',
      allList: state.cacheList,
    };
    financeRef.value?.init(data, { showAllFields: true, limitedFields: ['F_Money', 'F_Files', 'F_Description'] });
  }
  // 支出：打开收支记录新增，默认写入合同ID并设为“支出”
  function payHandle(record) {
    state.leftTreeActiveInfo = { F_ContractId: record.id, F_Type: '支出' };
    const data = {
      id: '',
      allList: state.cacheList,
    };
