            ...
            <template v-if="column.key === 'action' && !record.top">
              <div class="action-row">
                <div class="action-item">
                  <a-button type="link" class="action-button" @click="claimHandle(record)">领用</a-button>
                </div>
              </div>
            </template>
          </template>
...
<script lang="ts" setup>
  import { getList, exportData, batchDelete, moveToPublicSea as moveToPublicSeaApi, requisition } from './helper/api';
  ...
  import { BasicTable, useTable, ActionItem, TableActionType, SorterResult } from '@/components/Table';
  import Form from './Form.vue';
  import Detail from './Detail.vue';
  import { ExportModal, SuperQueryModal } from '@/components/CommonModal';
  ...
  // 领用处理（每行单一按钮）
  function claimHandle(record: any) {
    createConfirm({
      iconType: 'warning',
      title: '提示',
      content: '确定领用该客户吗？',
      onOk: () => {
        createMessage.info('处理中...');
        // 调用后端 Requisition 接口：POST /Requisition/{id}/{userId}
        const userId = userInfo?.userId;
        return requisition(record.id, userId)
          .then(res => {
            createMessage.success(res.msg || '领用成功');
            reload();
          })
          .catch(() => {
            createMessage.error('操作失败');
