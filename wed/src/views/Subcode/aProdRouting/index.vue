      ...
      <div class="jnpf-content-wrapper-content bg-white">
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          <template #tableTitle>
            ...
            <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="addDesign()" v-auth="'btn_add'">新增</a-button>
          </template>
...
<script lang="ts" setup>
  ...
  import DesignForm from './process.vue';
    ...
    return [
      {
        ...
        onClick: updateFlowHandle.bind(null, record),
        auth: 'btn_edit',
      },
      {
  ...
  // 流程编辑
  function updateFlowHandle(record) {
    // 不带工作流
    const data = {
      id: record.id,
      fullName: record.F_RoutingName,
    };
    openDesignFormModal(true, { id: data.id, fullName: data.fullName });
  }
