  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="title" width="40%" :minHeight="100" :showOkBtn="false" :canFullscreen="true">
    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Title')">
            <a-form-item name="F_Title" :labelCol="{ style: { width: '100px' } }">
              <template #label>标题</template>
              <JnpfInput
                v-model:value="dataForm.F_Title"
                :maskConfig="{
                  filler: '*',
                  maskType: 1,
                  prefixType: 1,
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Content')">
            <a-form-item name="F_Content" :labelCol="{ style: { width: '100px' } }">
              <template #label>验收内容</template>
              <p>{{ dataForm.F_Content }}</p>
            </a-form-item>
          </a-col>
