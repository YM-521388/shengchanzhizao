  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="title" width="70%" :minHeight="100" :showOkBtn="false" :canFullscreen="true">
    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_OutsourceId')">
            <a-form-item name="F_OutsourceId" :labelCol="{ style: { width: '100px' } }">
              <template #label>外协工单</template>
              <p class="leading-32px">{{ dataForm.F_OutsourceId }}</p>
              <ExtraRelationInfo
                :extraOptions="state.extraOptions.F_OutsourceId"
                :data="state.extraData.F_OutsourceId"
                v-if="state.extraOptions.F_OutsourceId?.length && state.extraData.F_OutsourceId && JSON.stringify(state.extraData.F_OutsourceId) !== '{}'" />
            </a-form-item>
          </a-col>
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_PassNum')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_FailNum')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_OutsourceType')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_SettleStatus')">
            <a-form-item name="F_SettleStatus" :labelCol="{ style: { width: '100px' } }">
              <template #label>结算状态</template>
              <p>{{ dataForm.F_SettleStatus }}</p>
            </a-form-item>
          </a-col>
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_SettleUnitPrice')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_SettleUserId')">
            <a-form-item name="F_SettleUserId" :labelCol="{ style: { width: '100px' } }">
              <template #label>结算人</template>
              <p>{{ dataForm.F_SettleUserId }}</p>
            </a-form-item>
          </a-col>
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_SettleTime')">
            <a-form-item name="F_SettleTime" :labelCol="{ style: { width: '100px' } }">
              <template #label>结算时间</template>
              <p>{{ dataForm.F_SettleTime }}</p>
            </a-form-item>
          </a-col>
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('WXYS_SettleFiles')">
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('WXYS_SettleDesc')">
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record, index }">
                  ...
                  <template v-if="column.key === 'F_AcceptId'">
                    <p class="leading-32px">{{ record.F_AcceptId }}</p>
                    <ExtraRelationInfo
                      :extraOptions="state.extraOptions.F_AcceptId"
                      :data="state.extraData.F_AcceptId"
                      v-if="state.extraOptions.F_AcceptId?.length && state.extraData.F_AcceptId && JSON.stringify(state.extraData.F_AcceptId) !== '{}'" />
                  </template>
...
<script lang="ts" setup>
  ...
  const tableField561a64Columns: any[] = computed(() => {
    ...
    list = list.filter(o => hasFormP('Deatil_WXYS-' + o.formP));
    let columnList = list;
  ...
  function getF_OutsourceIdExtraInfo() {
    if (!state.dataForm.F_OutsourceId_id) return;
    const paramList = getParamList('F_OutsourceId');
    const query = {
      ids: [state.dataForm.F_OutsourceId_id],
    ...
    getDataInterfaceDataInfoByIds('2014180492177444864', query).then(res => {
      ...
      state.extraData.F_OutsourceId = data;
    });
  }
</script>
