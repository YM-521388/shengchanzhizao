  ...
  <BasicModal
    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_DefectCode')">
            <a-form-item name="F_DefectCode" :labelCol="{ style: { width: '100px' } }">
              <template #label>不良品项编号</template>
              <JnpfInput
                v-model:value="dataForm.F_DefectCode"
          ...
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_DefectName')">
            <a-form-item name="F_DefectName" :labelCol="{ style: { width: '100px' } }">
              <template #label>不良品项名称</template>
              <JnpfInput
                v-model:value="dataForm.F_DefectName"
                placeholder="请输入"
                allowClear
                :style="{
                  width: '100%',
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    dataForm: {
      id: '',
      F_DefectCode: undefined,
      F_DefectName: undefined,
