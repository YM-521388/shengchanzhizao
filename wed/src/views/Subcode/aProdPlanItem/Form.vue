  ...
  <BasicModal v-bind="$attrs" @register="registerModal"  width="600px" :minHeight="100" okText="确定" cancelText="取消" @ok="handleSubmit" :closeFunc="onClose">
    ...
    <a-row class="dynamic-form ">
      <a-form :colon="false" layout="horizontal" labelAlign="right"  :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_ProdPlanId')" >
            <a-form-item name="F_ProdPlanId" :labelCol="{ style: { width: '100px' } }">
              <template #label>生产计划ID</template>
              <JnpfInput v-model:value="dataForm.F_ProdPlanId" placeholder='请输入' allowClear :style='{
  "width": "100%"
}'  :showCount='false'  />
            </a-form-item>
          </a-col>
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_GoodsId')" >
            <a-form-item name="F_GoodsId" :labelCol="{ style: { width: '100px' } }">
              <template #label>商品</template>
              <JnpfSelect v-model:value="dataForm.F_GoodsId" placeholder='请选择' :options="optionsObj.F_GoodsIdOptions" :fieldNames="optionsObj.F_GoodsIdProps" allowClear :style='{
  "width": "100%"
}'  />
            </a-form-item>
          </a-col>
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_Num')" >
            <a-form-item name="F_Num" :labelCol="{ style: { width: '100px' } }">
              <template #label>计划数量</template>
              <JnpfInputNumber v-model:value="dataForm.F_Num" placeholder='请输入' :controls="false" :style='{
  "width": "100%"
}'  />
            </a-form-item>
          </a-col>
          <a-col :span="24" class="ant-col-item" v-if="hasFormP('F_BOMId')" >
            <a-form-item name="F_BOMId" :labelCol="{ style: { width: '100px' } }">
              <template #label>商品BOM</template>
              <JnpfSelect v-model:value="dataForm.F_BOMId" placeholder='请选择' :options="optionsObj.F_BOMIdOptions" :fieldNames="optionsObj.F_BOMIdProps" allowClear :style='{
  "width": "100%"
}'  />
            </a-form-item>
          </a-col>
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    optionsObj:{
      F_GoodsIdProps: {'label':'F_GoodsName','value':'id'},
    ...
    let templateJson = [
  {
    ...
    getDataInterfaceRes('2012085692393459712', query).then(res => {
      let data = res.data;
      state.optionsObj.F_GoodsIdOptions = Array.isArray(data) ? data : [];
    });
  }
