<template>
  <div class="jnpf-content-wrapper">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_State'">
                <a-tag
                  :color="
                    record.F_StateCode == 0
                      ? 'warning'
                      : record.F_StateCode == 1
                      ? 'warning'
                      : record.F_StateCode == 2
                      ? 'blue'
                      : record.F_StateCode == 3
                      ? 'orange'
                      : record.F_StateCode == 4
                      ? 'red'
                      : record.F_StateCode == 5
                      ? 'success'
                      : record.F_StateCode == 6
                      ? 'purple'
                      : record.F_StateCode == 7
                      ? 'error'
                      : 'default'
                  ">
                  {{ record.F_State }}
    ...
    <ReceiveForm ref="receiveRef" @reload="reload" />
    <ReceiptForm ref="receiptRef" @reload="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  function getDropDownActions(record): ActionItem[] {
    return [
      {
        label: '派单',
        ifShow: record.F_StateCode == 0,
        onClick: stateHandle.bind(null, record, '1'),
        auth: 'btn_receiveOrder',
      },
      {
        label: '接单',
        ifShow: record.F_StateCode == 1,
        modelConfirm: {
          content: '请确定是否接受维修单【' + record.F_RepairNo + '】?',
          onOk: stateHandle.bind(null, record, '2'),
        },
        auth: 'btn_dispatchOrder',
      },
      {
        label: '暂停',
        ifShow: record.F_StateCode != 7 && record.F_StateCode != 6 && record.F_StateCode != 4 && record.F_StateCode != 5,
        onClick: stateHandle.bind(null, record, '6'),
        auth: 'btn_pause',
      },
      {
        label: '继续',
        ifShow: record.F_StateCode == 6,
        modelConfirm: {
          content: '请确定是否继续维修工单吗?',
          onOk: stateHandle.bind(null, record, '8'),
        },
        auth: 'btn_continue',
      },
      {
        label: '取消',
        ifShow: record.F_StateCode != 7 && record.F_StateCode != 4 && record.F_StateCode != 5,
        modelConfirm: {
          content: '请确定是否取消维修工单？已经取消的工单无法继续处理，该操作不可逆，请谨慎操作吗。',
          onOk: stateHandle.bind(null, record, '7'),
        },
        auth: 'btn_cancel',
      },
      {
        label: '回执',
        ifShow: record.F_StateCode == 2 && (record.F_IsSolved == '' || record.F_IsSolved == null),
        onClick: stateHandle.bind(null, record, '9'),
        auth: 'btn_receipt',
      },
      // {
      //   label: '转派',
      //   ifShow: record.F_StateCode == 2 && (record.F_IsSolved == '' || record.F_IsSolved == null),
      //   onClick: stateHandle.bind(null, record, '11'),
      //   auth: 'btn_receipt',
      // },
      {
        label: '审核通过',
        ifShow: record.F_StateCode == 2 && record.F_IsSolved != '' && record.F_IsSolved != null,
        onClick: stateHandle.bind(null, record, '10'),
        auth: 'btn_approve',
      },
      {
        label: '审核退回',
        ifShow: record.F_StateCode == 2 && record.F_IsSolved != '' && record.F_IsSolved != null,
        onClick: stateHandle.bind(null, record, '4'),
        auth: 'btn_reject',
      },
    ];
  }
  // 修改状态
  function stateHandle(record, state) {
    if (state == '6') {
      const data = {
        id: record.id,
        type: state,
      ...
      redeployRef.value?.init(data);
    } else {
      handleCheck(record.id, state).then(res => {
