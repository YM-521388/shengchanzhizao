...
enum Api {
  Prefix = '/api/example/AMaintRepair',
}
...
export function receiveUpdate(data) {
  return defHttp.put({ url: Api.Prefix + `/ReceiveUpdate/` + data.id, data });
}
// 回执
export function receiptUpdate(data) {
  return defHttp.put({ url: Api.Prefix + `/ReceiptUpdate/` + data.id, data });
}
