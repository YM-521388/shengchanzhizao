          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_Type'">
                <a-tag
                  :color="record[column.dataIndex] === '支出' ? 'success' : 'error'"
                  :title="record[column.dataIndex] !== '支出' ? `实际值: ${record[column.dataIndex] || '空值'}` : undefined">
                  {{ record[column.dataIndex] === '支出' ? '支出' : '收入' }}
                </a-tag>
              </template>
