  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="title" width="70%" :minHeight="100" :showOkBtn="false" :canFullscreen="true">
    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('F_CheckNo')">
            <a-form-item name="F_CheckNo" :labelCol="{ style: { width: '100px' } }">
              <template #label>盘库单号：</template>
              <p>{{ dataForm.F_CheckNo }}</p>
            </a-form-item>
          </a-col>
...
<script lang="ts" setup>
  ...
  const allColumns: BasicColumn[] = [
    ...
    { title: '商品规格', dataIndex: 'F_Specification', resizable: true, width: 150 },
    { title: '盘点前数量', dataIndex: 'F_CheckQtyBeforeInfo', resizable: true, width: 120 },
    { title: '已盘点数量', dataIndex: 'F_CheckQtyDoneInfo', resizable: true, width: 120 },
    { title: '差异数量', dataIndex: 'F_DifferencesInfo', resizable: true, width: 100 },
    // { title: '单位', dataIndex: 'F_UnitTwo', resizable: true, width: 100 },
    { title: '备注', dataIndex: 'F_Description', resizable: true },
  ];
  const columns = computed(() => allColumns.filter(col => typeof col.dataIndex === 'string' && hasFormP(('Detail_KCPD-' + col.dataIndex) as string)));

  const [registerTable, { reload }] = useTable({
    api: getGoodsList,
    beforeFetch: data => {
      data.F_CheckId = state.dataForm.id;
      return data;
    },
    columns,
    useSearchForm: false,
  ...
  const columnsLog = computed(() =>
    allColumnsLog.filter(col => typeof col.dataIndex === 'string' && hasFormP(('tableField960db3-' + col.dataIndex) as string)),
  );
