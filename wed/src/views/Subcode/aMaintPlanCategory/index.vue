...
<script lang="ts" setup>
  ...
  function handleLeftTreeSelect(id, _node, nodePath) {
    ...
    // 左侧树是其他
    queryJson = { F_ParentId: state.treeActiveId };
    leftTreeActiveInfo = { F_ParentId: state.treeRelationObj?.multiple ? [state.treeActiveId] : state.treeActiveId };
  ...
  function getSearchSchemas() {
    // 有左侧树，有关联字段
    for (let i = 0; i < searchInfo.superQueryJson.length; i++) {
      const e = searchInfo.superQueryJson[i];
      if (e.id === 'F_ParentId') {
