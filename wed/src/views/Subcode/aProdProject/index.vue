        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_ProjectType'">
                <a-tag :color="record.F_ProjectTypeCode == '0' ? 'success' : record.F_ProjectTypeCode == '1' ? 'warning' : 'blue'">
                  {{ record.F_ProjectType }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_Status'">
                <a-tag :color="record.F_Status == '已完成' ? 'success' : 'warning'">
                  {{ record.F_Status }}
                </a-tag>
              </template>
            </template>
