    ...
    <div class="jnpf-content-wrapper-center">
      ...
      <div class="jnpf-content-wrapper-content bg-white">
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="addHandle()" v-auth="'btn_add'">{{ t('生成') }}</a-button>
...
<script lang="ts" setup>
  ...
  import { deleteQr } from '@/views/Subcode/aMaintDevice/helper/api';
  ...
  function handleState(record) {
    createConfirm({
      iconType: 'warning',
      title: '确认为二维码编号：' + record.F_Code + '+解除绑定吗？',
      onOk: () => {
        deleteQr(record.id).then(res => {
  ...
  async function downloadQrCode(record) {
    try {
      if (!record.F_Code) {
        createMessage.warning('设备编码为空,无法生成二维码');
        return;
      }
      const url = await QRCode.toDataURL(record.F_Code, {
        width: 300,
        margin: 2,
  ...
  async function downloadAllQrCode() {
    try {
      ...
      createMessage.loading('正在生成二维码,请稍候...');
      ...
      for (let i = 0; i < dataToDownload.length; i++) {
        ...
        try {
          ...
          console.error(`生成 ${record.F_Code} 二维码失败:`, error);
        }
      }

      const content = await zip.generateAsync({ type: 'blob' });
      downloadByData(content, '设备二维码.zip', 'application/zip');
      createMessage.success('下载成功');
    } catch (error) {
      createMessage.error('下载失败');
      console.error(error);
