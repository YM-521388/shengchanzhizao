      ...
      <div class="jnpf-content-wrapper-content bg-white">
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          <template #tableTitle>
            ...
            <a-button type="link" preIcon="icon-ym icon-ym-btn-download" @click="handleDownloadTemplate()" v-auth="'btn_add'">{{
              t('common.downloadTemplate', '下载模版')
            }}</a-button>
            <!-- <a-button
              type="link"
...
<script lang="ts" setup>
  import { getList, del, exportData, batchDelete, getTemplateDownload } from './helper/api';
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: t('common.importText', '导入'),
        onClick: openImportModal.bind(null, true, { url: 'example/ABom', menuId: searchInfo.menuId, recordId: record.id }),
        auth: 'btn_upload',
      },
    ];
  }
  ...
  function handleDownloadTemplate() {
    getTemplateDownload({ menuId: searchInfo.menuId }).then(res => {
