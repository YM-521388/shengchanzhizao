      ...
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" ref="formRef">
        <a-row :gutter="15">
          ...
          <a-col :span="24" class="ant-col-item">
            <a-form-item label="">
              <JnpfGroupTitle :content="t('入库商品')" :bordered="true" />
...
<script lang="ts" setup>
  ...
  const allColumns: BasicColumn[] = [
    { title: '工单号', dataIndex: 'F_WorkOrderId', resizable: true, width: 150 },
    ...
    // { title: '计划数量', dataIndex: 'F_PlanQty', resizable: true, width: 100 },
    // { title: '已入库数量', dataIndex: 'F_YseNum', resizable: true, width: 100 },
    { title: '入库数量', dataIndex: 'F_NumInfo', resizable: true, width: 100 },
    { title: '成本单价(元)', dataIndex: 'F_Price', resizable: true, width: 100 },
    // { title: '单位', dataIndex: 'F_UnitTwo', resizable: true, width: 100 },
    { title: '入库仓库', dataIndex: 'F_WarehouseId', resizable: true, width: 200 },
    { title: '备注', dataIndex: 'F_Description', resizable: true },
  ];

  const columns = computed(() => allColumns.filter(col => typeof col.dataIndex === 'string' && hasFormP(('CPRKXQ-' + col.dataIndex) as string)));
  const [registerTable, { reload }] = useTable({
    api: getGoodsList,
    beforeFetch: data => {
      data.F_StockFgId = state.dataForm.id;
      return data;
    },
    columns,
    useSearchForm: false,
  ...
  const columnsLog = computed(() => allColumnsLog.filter(col => typeof col.dataIndex === 'string' && hasFormP(('CPRKXQLOG-' + col.dataIndex) as string)));
