  ...
  <BasicModal
    ...
    <a-row class="dynamic-form">
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_ProcessNo')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_GoodsId')">
            <a-form-item name="F_GoodsId" :labelCol="{ style: { width: '100px' } }">
              ...
              <JnpfPopupSelect
                ...
                propsValue="id"
                :pageSize="20"
                :disabled="dataForm.id || state.title == '创建加工单'"
                popupType="dialog"
                popupTitle="选择数据"
                popupWidth="1000px"
                hasPage
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_BOMId')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_PlanQty')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_DeliveryDate')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_Priority')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_PlanStartDate')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_PlanEndDate')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_DrawingFile')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="hasFormP('F_CustomerName')">
            <a-form-item name="F_CustomerName" :labelCol="{ style: { width: '100px' } }">
              ...
              <JnpfSelect
                ...
                placeholder="请选择"
                :options="optionsObj.F_CustomerNameOptions"
                :fieldNames="optionsObj.F_CustomerNameProps"
                allowClear
                showSearch
                :style="{
                  width: '100%',
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_DoorPlateThickness')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_BoxPlateThickness')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_InstallOrBeam')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_LockSet')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_Hinge')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_Color')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_Type')">
          ...
          <a-col :span="8" class="ant-col-item" v-if="formHasP('F_RoutingId')">
            <a-form-item name="F_RoutingId" :labelCol="{ style: { width: '130px' } }">
              <template #label>选择工艺路线</template>
              <JnpfSelect
                v-model:value="dataForm.F_RoutingId"
                placeholder=""
          ...
          <a-col :span="12" class="ant-col-item" v-if="formHasP('F_BOMImages')">
          ...
          <a-col :span="12" class="ant-col-item" v-if="formHasP('F_Description')">
          ...
          <a-col :span="24" class="ant-col-item">
            <a-form-item label="">
              ...
              <a-col :span="24" class="ant-col-item" v-if="formHasP('GoodsList')">
...
<script lang="ts" setup>
  ...
  /** 表单权限：弹窗从其他页面打开时用传入的 menuId，否则用当前路由的 modelId */
  function formHasP(field: string) {
    const modelId = state.formMenuId || (route.meta?.modelId as string) || '';
    return hasFormP(field, true, modelId);
  }
  ...
  const tableField449e6cColumns = computed<any[]>(() => {
    let list = [
      {
        title: '排序',
    ...
    list = list.filter(o => formHasP('tableField449e6c-' + o.formP));
    let columnList = list;
  ...
  const state = reactive<State>({
    ...
    optionsObj: {
      ...
      F_CustomerNameProps: { label: 'F_CustomerName', value: 'F_Id' },
  ...
  const { title, continueText, dataForm, submitType, dataRule, optionsObj } = toRefs(state);

  const _getPrevDisabled = computed(() => state.currIndex === 0);
  const _getNextDisabled = computed(() => state.currIndex === state.allList.length - 1);
    ...
    try {
      ...
      if (invalidRows.length > 0) {
        const errorMsg = invalidRows
          .map(item => {
            const goods = state.optionsObj.tableField449e6c_F_GoodsIdOptions.find(g => g.id === item.F_GoodsId);
  ...
  function getF_CustomerNameOptions() {
    let templateJson = [];
    let query = {
    ...
    getDataInterfaceRes('2009458830353764352', query).then(res => {
      let data = res.data;
      state.optionsObj.F_CustomerNameOptions = Array.isArray(data) ? data : [];
    });
  }
  ...
  function GoodsListBtn(_val, option) {
    /* 兜底 */
  ...
  function F_GoodsIdChange(_val, _option) {
    state.dataForm.F_BOMId = undefined;
    getF_BOMIdOptions();
  }
  ...
  function F_BomChange(val, _option) {
    getBOMGoodsList(val).then(res => {
      ...
      let nextSort = 1;
      res.data.forEach(o => {
        getGoodsUnit(o.F_GoodsId).then(res => {
          const newRow = {
            jnpfId: buildUUID(),
