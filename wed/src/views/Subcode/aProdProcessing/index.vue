<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_State'">
                <a-tag
                  :color="
                    record.F_StateId == 2
                      ? 'success'
              ...
              <template v-if="column.key === 'F_SequenceNo' && record.F_SequenceNo">
                <a-tag color="green">
                  {{ record.F_SequenceNo }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_Priority' && record.F_Priority">
                <a-tag
                  :color="record.F_PriorityCode == 1 ? 'success' : record.F_PriorityCode == 2 ? 'warning' : record.F_PriorityCode == 0 ? 'error' : 'default'">
                  {{ record.F_Priority }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_Source' && record.F_Source">
                <a-tag :color="record.F_Source == '合同' ? 'warning' : 'success'">
                  {{ record.F_Source }}
                </a-tag>
              </template>
            </template>
    ...
    <StockFgForm ref="stockFgRef" @reload="reload" />
    <StockProdForm ref="stockProdRef" @reload="reload" />
    <ReturnPrdForm ref="returnPrdRef" @reload="reload" />
    <ProdTaskFrom ref="prodTaskRef" @reload="reload" />
  </div>
</template>

<script lang="ts" setup>
  import { getList, del, exportData, batchDelete, handleCheck } from './helper/api';
  ...
  import StockFgForm from '@/views/Subcode/aPuStockFg/Form.vue';
  import StockProdForm from '@/views/Subcode/aPuOutStockProd/Form.vue';
  import ReturnPrdForm from '@/views/Subcode/aPuReturnPrd/Form.vue';
  import ProdTaskFrom from './ProdTaskFrom.vue';
  ...
  const prodTaskRef = ref<any>(null);
  const returnPrdRef = ref<any>(null);
  const stockProdRef = ref<any>(null);
  const stockFgRef = ref<any>(null);
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: '生产任务',
        onClick: stockProdTask.bind(null, record),
        auth: 'btn_task',
      },
    ];
  }
  ...
  function getDropDownActions(record): ActionItem[] {
    return [
      {
        label: '未开始',
        ifShow: record.F_StateId == '1',
        onClick: handleState.bind(null, record.id, '0'),
        auth: 'btn_notStart',
      },
      {
        label: '开始',
        ifShow: record.F_StateId == '0',
        onClick: handleState.bind(null, record.id, '1'),
        auth: 'btn_start',
      },
      {
        label: '完成',
        ifShow: record.F_StateId == '0' || record.F_StateId == '1',
        onClick: handleState.bind(null, record.id, '2'),
        auth: 'btn_complete',
      },
      {
        label: '生产中',
        ifShow: record.F_StateId == '2',
        onClick: handleState.bind(null, record.id, '6'),
        auth: 'btn_producing',
      },
      {
        label: '取消',
        ifShow: record.F_StateId != '4',
      ...
      {
        label: '继续',
        ifShow: record.F_StateId == '3',
        onClick: handleState.bind(null, record.id, '5'),
        auth: 'btn_continue',
      },
      {
        label: t('生产出库'),
        onClick: stockProdHandle.bind(null, record),
        auth: 'btn_prodOut',
      },
      {
        label: t('成品入库'),
        onClick: stockFgHandle.bind(null, record),
        auth: 'btn_fgIn',
      },
      {
        label: t('生产退料'),
        onClick: returnPrdHandle.bind(null, record),
        auth: 'btn_prodReturn',
      },
    ];
  }
  // 生产任务
  function stockProdTask(record) {
    // 不带工作流
    const data = {
      id: record.id,
      menuId: searchInfo.menuId,
      F_ProcessNo: record.F_ProcessNo,
      taskId: '',
    };
    prodTaskRef.value?.init(data);
  }
  ...
  // 成品入库
  function stockFgHandle(record) {
    // 不带工作流
    const data = {
      id: '',
      F_Method: '0',
      F_ProcessId: record.id,
      F_Type: '0',
      fullName: '成品入库',
    };
    stockFgRef.value?.init(data);
  }
  // 生产出库
  function stockProdHandle(record) {
    // 不带工作流
    const data = {
      id: '',
      F_Method: '0',
    ...
    stockProdRef.value?.init(data);
  }
  // 生产退料
  function returnPrdHandle(record) {
    // 不带工作流
    const data = {
      id: '',
      F_Method: '0',
