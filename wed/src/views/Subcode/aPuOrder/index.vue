<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_CheckState' && record.F_CheckState">
                <a-tag :color="record.F_CheckStateInfo == '1' ? 'success' : record.F_CheckStateInfo == '2' ? 'error' : 'default'">
                  {{ record.F_CheckState }}
                </a-tag>
              </template>
            </template>
    ...
    <PayForm ref="payFormRef" :ignorePermissions="true" />
...
<script lang="ts" setup>
  ...
  import PayForm from '../aPuOrderPay/Form.vue';
  ...
  function getDropDownActions(record): ActionItem[] {
    return [
      ...
      {
        label: '通过',
        disabled: record.F_CheckStateInfo != '0',
  ...
  function payHandle(record) {
    ...
    (payFormRef.value as any)?.init(data);
  }
  // 新增
  function addHandle() {
    const data = {
