        ...
        <BasicTable
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.jnpfKey === 'input' && column.dataIndex !== 'F_State'">
...
<script lang="ts" setup>
  import { getList, del, batchDelete, update, completeOutsource, handleCheck } from './helper/api';
  ...
  function getDropDownActions(record): ActionItem[] {
    return [
      ...
      // {
      //   label: '撤回',
      //   ifShow: record.F_StateId == '1' || record.F_StateId == '2',
      ...
      {
        label: '取消',
        ifShow: record.F_StateId != '4' && record.F_StateId != '2',
...
<style scoped>
  ...
  .status-production {
    ...
    border-color: #ffc069;
    background: #fff7e6;
    box-shadow: 0 0 0 2px rgba(255, 192, 92, 0.06);
  }
  .status-success {
    color: #237804;
    border-color: #b7eb8f;
    background: #f6ffed;
    box-shadow: 0 0 0 2px rgba(183, 235, 143, 0.06);
  }
</style>
