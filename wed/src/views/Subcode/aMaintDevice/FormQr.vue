<template>
  <BasicModal v-bind="$attrs" @register="registerModal" width="500px" :minHeight="100" okText="确定" cancelText="取消" @ok="handleSubmit" :closeFunc="onClose">
    <template #title>
      <a-space :size="10">
      ...
      <a-form :colon="false" layout="horizontal" labelAlign="right" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          <a-col :span="24" class="ant-col-item">
            <a-form-item name="F_DeviceQrCode" :labelCol="{ style: { width: '100px' } }">
              <template #label>设备二维码</template>
              <JnpfSelect
                v-model:value="dataForm.F_DeviceQrCode"
                placeholder="请选择"
                :options="optionsObj.F_DeviceQrCodeOptions"
                :fieldNames="optionsObj.F_DeviceQrCodeProps"
                allowClear
                showSearch
                :style="{
                  width: '100%',
...
<script lang="ts" setup>
  import { updateQr, getInfo } from './helper/api';
  ...
  const state = reactive<State>({
    ...
    dataRule: {},
    optionsObj: {
      F_DeviceQrCodeProps: { label: 'F_Code', value: 'F_Code' },
    },
    currVmodel: '',
    currTableConf: {},
    tableRows: {},
  ...
  function getF_DeviceQrCodeOptions() {
    let templateJson = [];
    let query = {
      paramList: getParamList(templateJson, state.dataForm),
    };
    getDataInterfaceRes('2016439739686719488', query).then(res => {
      let data = res.data;
      state.optionsObj.F_DeviceQrCodeOptions = Array.isArray(data) ? data : [];
    });
  }
