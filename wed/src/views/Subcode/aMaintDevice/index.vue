<template>
  <div class="jnpf-content-wrapper">
        ...
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange" @resizeColumn="handleResizeColumn">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_DeviceStatus'">
                <a-tag :color="record.F_DeviceStatusCode == 0 ? 'success' : record.F_DeviceStatusCode == 1 ? 'warning' : 'default'">
                  {{ record.F_DeviceStatus }}
                </a-tag>
              </template>
              <template v-if="column.key === 'F_EnabledMark'">
                <a-tag :color="record.F_EnabledMark == '启用' ? 'success' : 'error'">
                  {{ record.F_EnabledMark }}
                </a-tag>
              </template>
            </template>
    ...
    <FormQr ref="qrRef" @reload="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: t('设备二维码'),
        onClick: qrHandle.bind(null, record),
        auth: 'btn_qr',
      },
    ];
  }
  // 设备二维码
  function qrHandle(record) {
    // 不带工作流
    const data = {
      id: record.id,
    };
    qrRef.value?.init(data);
  }
  // 新增
  function addHandle() {
    const data = {
