          ...
          <template #bodyCell="{ column, record }">
            <template v-if="!(record.top || column.id?.includes('-'))">
              ...
              <template v-if="column.key === 'F_SettleStatus'">
                <a-tag
                  :color="record[column.dataIndex] === '已结算' ? 'success' : 'error'"
                  :title="record[column.dataIndex] !== '已结算' ? `实际值: ${record[column.dataIndex] || '空值'}` : undefined">
                  {{ record[column.dataIndex] === '已结算' ? '已结算' : '未结算' }}
                </a-tag>
              </template>
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    return [
      {
        label: '结算',
        disabled: record.F_SettleStatus == '已结算',
        onClick: () => updateHandle(record, true),
        auth: 'btn_edit',
      },
    ];
  }
  ...
  // 编辑
  function updateHandle(record, isSettle = false) {
    // 不带工作流
    const data = {
      id: record.id,
      menuId: searchInfo.menuId,
