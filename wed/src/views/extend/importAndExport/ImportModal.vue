<template>
  <BasicModal
    ...
    <div class="import-main" v-show="activeStep == 1">
      <a-table :data-source="list" :columns="columns" size="small" :pagination="false" :scroll="{ x: 'max-content', y: '420px' }" class="import-preview-table">
...
<script lang="ts" setup>
  ...
  const { activeStep, fileName, fileList, btnLoading, list, result, resultList, skipPreview } = toRefs(state);
  ...
  function handleSubmit() {
    ...
    importData(query)
      .then(res => {
        ...
        state.activeStep += 1;
        if (state.skipPreview) state.activeStep = 2;
        state.btnLoading = false;
      })
      .catch(() => {
        state.btnLoading = false;
