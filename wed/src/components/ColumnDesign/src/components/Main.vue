      ...
      <a-table :data-source="columnData.searchList" :columns="searchColumns" size="small" :pagination="false" rowKey="id" class="search-table">
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.key === 'label'">
            <jnpf-i18n-input v-model:value="record.label" v-model:i18n="record.labelI18nCode" placeholder="请输入" allowClear />
          </template>
...
<script lang="ts" setup>
  ...
  function init(list) {
    ...
    state.tabRelationFieldOptions = list.filter(
      o => o.id.indexOf('-') < 0 && ['radio', 'select'].includes(o.__config__.jnpfKey) && o.__config__.dataType != 'dynamic' && !o.multiple,
    );
  ...
  function updateTreeRelationInfo() {
    ...
    for (let i = 0; i < unref(selectOptions).length; i++) {
      ...
      if (e.id === state.columnData.treeRelation && e.__config__?.jnpfKey && ['organizeSelect', 'depSelect'].includes(e.__config__.jnpfKey)) {
  ...
  function onRelationChange(val, data) {
    if (data.__config__?.jnpfKey && ['organizeSelect', 'depSelect'].includes(data.__config__.jnpfKey)) {
