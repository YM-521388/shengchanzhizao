<template>
  <div class="common-container">
    ...
    <ExtraRelationInfo :extraOptions="extraOptions" :data="extraData" v-if="extraOptions.length && JSON.stringify(extraData) !== '{}'" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      ...
      if (props.propsValue) query = { ...query, propsValue: props.propsValue };
  ...
  async function openSelectModal() {
    if (props.disabled) {
      ...
      getConfigData(props.modelId).then(res => {
        ...
        const data = { id: props.value, formConf, modelId: props.modelId, propsValue: props.propsValue };
  ...
  function handleSubmit() {
    ...
    innerValue.value = unref(selectRow)[props.propsValue];
    emit('update:value', unref(selectRow)[props.propsValue]);
