<template>
  <div class="upload-file-container" :class="$attrs.class">
    ...
    <FileViewer
      ...
      :watermark="{ type: 'text', content: '内部文件 禁止外传', color: 'rgba(0,0,0,0.1)', opacity: 0.6, gapX: 80, gapY: 60 }" />
...
<script lang="ts" setup>
  ...
  function handlePreview(file: fileItem) {
    ...
    // 文件预览 - 使用 FileViewer 全屏预览
    const fileUrl = file.url || '';
    const fullUrl = fileUrl.startsWith('http') ? fileUrl : apiUrl.value + fileUrl;
    fileViewerUrl.value = fullUrl;
    fileViewerName.value = file.name || '未知文件';
