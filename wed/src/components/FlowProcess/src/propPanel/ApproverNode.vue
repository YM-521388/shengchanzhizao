    ...
    <a-form :colon="false" :model="formConf" class="config-content" layout="vertical">
      <template v-if="activeKey === 1">
        ...
        <a-form-item v-if="formConf.counterSign == 1">
          <template #label>计算方式<BasicHelp :text="['实时计算：审批过程中实时进行规则判断', '延后计算：在所有审批人审批完成后进行规则判断']" /></template>
      ...
      <template v-if="activeKey === 2">
        <a-form-item label="处置操作">
          ...
          <div class="btn-cell">
            <a-checkbox v-model:checked="formConf.hasFreeApproverBtn" @change="onHasFreeApproverBtnChange(formConf.hasFreeApproverBtn)">加签</a-checkbox>
            <a-input v-model:value="formConf.freeApproverBtnText" placeholder="请输入" />
          </div>
          <div class="btn-cell" v-if="formConf.hasFreeApproverBtn">
        ...
        <a-form-item label="辅助信息">
          <a-button preIcon="icon-ym icon-ym-btn-add" @click="openAuxiliaryMessageModal">辅助信息设置</a-button>
        </a-form-item>
      </template>
      <template v-if="activeKey === 4">
        ...
        <a-form-item class="!-mt-12px" v-if="formConf.backFuncConfig.on">
          ...
          <select-interface
            ...
            :templateJson="formConf.backFuncConfig.templateJson"
            :fieldOptions="funcOptions"
            :sourceType="3"
            showSystemFullLabel
            isFlow
      ...
      <template v-if="activeKey === 6">
        ...
        <a-form-item v-if="formConf.overTimeConfig.on === 1">
          ...
          <div v-if="formConf.overTimeConfig.overAutoTransfer" class="transfer-tip !mt-12px">转审人设置</div>
          <jnpf-radio
        ...
        <template v-if="formConf.overTimeConfig.overAutoTransfer && formConf.overTimeConfig.on === 1">
          <a-form-item class="!mb-0" v-if="formConf.overTimeConfig.overTimeType === 6">
            <jnpf-users-select v-model:value="formConf.overTimeConfig.reApprovers" buttonType="button" modalTitle="添加转审人" :multiple="false" />
          </a-form-item>
...
<script lang="ts" setup>
  import { reactive, toRefs, inject, computed, unref, watch, h } from 'vue';
  ...
  import { SelectInterfaceBtn, SelectInterface } from '@/components/Interface';
