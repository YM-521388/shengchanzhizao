        ...
        <draggable v-model="activeData.option.columnOptions" :animation="300" group="selectItem" handle=".option-drag" itemKey="id">
          <template #item="{ element, index }">
            <div class="select-item">
              ...
              <jnpf-i18n-input v-model:value="element.label" v-model:i18n="element.labelI18nCode" placeholder="列名" />
              <a-auto-complete
...
<script lang="ts" setup>
  ...
  function initMappingConfig() {
    ...
    if (['barChart', 'pieChart', 'lineChart'].includes(props.activeData.jnpfKey)) return mappingList.filter(o => [1, 2, 3].includes(o.id));
    if (['radarChart'].includes(props.activeData.jnpfKey)) return mappingList.filter(o => [1, 2, 3, 4].includes(o.id));
    if (['timeAxis'].includes(props.activeData.jnpfKey)) return mappingList.filter(o => [2, 5].includes(o.id));
    if (['mapChart'].includes(props.activeData.jnpfKey)) return mappingList.filter(o => [2, 3, 6, 7].includes(o.id));
  }
