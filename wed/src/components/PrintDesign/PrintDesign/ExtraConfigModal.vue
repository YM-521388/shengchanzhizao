<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="属性配置" @ok="handleSubmit" destroyOnClose class="extra-config-modal">
    <div class="extra-config-modal-body" :style="{ 'min-height': activeData.type === 'select' ? '300px' : '150px' }">
      <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
        <template v-if="activeData.type === 'select'">
          ...
          <div v-if="activeData.config.dataType === 'dictionary'">
            ...
            <a-form-item label="存储字段">
              <jnpf-select v-model:value="activeData.config.propsValue" placeholder="请选择" :options="valueOptions" />
            </a-form-item>
          </div>
        </template>
        <a-form-item label="日期类型" v-if="activeData.type === 'date'">
          <jnpf-select v-model:value="activeData.config.format" placeholder="请选择" :options="dateFormatOptions" />
        </a-form-item>
        <a-form-item label="日期类型" v-if="activeData.type === 'time'">
          <jnpf-select v-model:value="activeData.config.format" placeholder="请选择" :options="timeFormatOptions" />
        </a-form-item>
        ...
        <a-form-item label="" v-if="['organizeSelect', 'depSelect', 'userSelect'].includes(activeData.type)">
          <a-checkbox v-model:checked="activeData.isIncludeSubordinate">
            查询当前{{ activeData.type === 'organizeSelect' ? '组织及子组织' : activeData.type === 'depSelect' ? '部门及子部门' : '用户及下属' }}
          </a-checkbox>
        </a-form-item>
      </a-form>
