<template>
  ...
  <FlowFormModalProdTask ref="prodTaskNode" />
...
<script lang="ts" setup>
  ...
  import FlowFormModalProdTask from '../propPanel/components/FlowFormModalProdTask.vue';
  ...
  const prodTaskNode = ref<any>(null);
  ...
  function handleInitListeners(bpmn, modeling) {
    ...
    eventBus.on('element.click', e => {
      ...
      handleElementClick(element);
        ...
        prodTaskNode.value?.hideModal()

      let currentNodeData = props.flowNodes[element.id];
      if (props.formType === "detail"&&element.wnType==="approver") {
        nextTick(() => {
        state.element = e.element
        prodTaskNode.value?.init(currentNodeData)
      })
    }
    });
    ...
    bpmn.on('copyPaste.copyElement', ({ element }) => {
      if (element.type === bpmnStart) {
        Message.warning('开始节点不能复制');
        return false;
      }
      if (element.type === bpmnEnd) {
        Message.warning('结束节点不能复制');
        return false;
      }
  ...
  function handleShapeAdded(element) {
    ...
    if (currentData) {
      ...
      // 添加工序节点时，从连接的源节点获取parentId
      if (element.wnType === typeProcessing && element.incoming && element.incoming.length > 0) {
        const sourceElement = element.incoming[0].source;
        if (sourceElement && sourceElement.id) {
          data.parentId = sourceElement.id;
        }
      }
  ...
  function handleConnectionAdded(e, modeling) {
    ...
    // 当新的连线被添加时，如果目标节点是工序节点，设置parentId为源节点ID
    if (e.element && e.element.type === bpmnSequenceFlow && e.element.target && e.element.source) {
      const targetElement = e.element.target;
      const sourceElement = e.element.source;
      if (targetElement.wnType === typeProcessing && sourceElement.id) {
        const currentData = state.jnpfData.getValue(targetElement.id) || {};
        state.jnpfData.setValue(targetElement.id, { ...currentData, parentId: sourceElement.id });
      }
    }
  }
  ...
  async function handleCommandStackCanExecute(e, modeling) {
    ...
    if (e.command == 'elements.delete') {
      ...
      hasStartElement && Message.warning('开始节点不能删除');
  ...
  function getNodeName(type) {
    if (type == typeStart) return '开始';
    if (type == typeTask) return '工序节点';
    if (type == typeSubFlow) return '子流程';
    if (type == typeEnd) return '结束';
  }
  function getElement() {
