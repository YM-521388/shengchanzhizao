...
const CustomizeContextPad = (contextPadProvider: any, element: any) => {
  ...
  if (typeConfig[type]) {
    ...
    const { connect, end, approver, subFlow, del, trigger, copy } = jnpfConfigBpmnContextPad;
    // 根据类型 判断contextPad
    if (type === shapeType) {
      if (contextPad) {
      ...
      function appendAction(type: any, name: any, className: any, title: any, wnType: any, options?: any) {
        const appendStart = (event: any, element: any) => {
          ...
          if (type === typeCopy) {
            ...
            let hasTaskSet = new Set([typeSubFlow, typeProcessing]);
        ...
        const autoPlaceAppend = async (_event: any, element: any) => {
          let hasTaskSet = new Set([typeSubFlow, typeTrigger, typeProcessing]);
