    ...
    if (allElements && allElements.length > 0) {
      ...
      if (newElementData) {
        ...
        if (newElementData['endevent']) {
          // 允许多个结束节点，只校验每个结束节点的基本规则
          for (let i = 0; i < newElementData['endevent'].length; i++) {
            let endNode = newElementData['endevent'][i];
            let outLine = endNode.outgoing;
            let inLine = endNode.incoming; // 进线
