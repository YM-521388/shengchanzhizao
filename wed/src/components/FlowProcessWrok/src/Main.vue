<template>
  <div class="center-container">
    ...
    <template v-if="isPreview">
      <div class="tips">
        <div class="tips-item"><span class="icon notstart">●</span>未开始</div>
        <div class="tips-item"><span class="icon curr">●</span>进行中</div>
        <div class="tips-item"><span class="icon past">●</span>已完成</div>
        <div class="tips-item"><span class="icon paused">●</span>已暂停</div>
      </div>
    ...
    <div class="center-container-center">
      <JnpfBpmn v-bind="getBpmnBindValue" ref="jnpfBpmnRef" @viewSubFlow="viewSubFlow" />
    </div>
  </div>
</template>
<script lang="ts" setup>
  ...
  import JnpfBpmn from './bpmn/index.vue';
  ...
  const props = defineProps(['flowInfo', 'nodeList', 'flowState', 'versionList', 'type', 'openPreview', 'isPreview', 'noShowScale', 'lineKeyList','formType']);
