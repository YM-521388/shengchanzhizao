<template>
  <section class="common-pane">
    ...
    <GlobalProperties v-bind="getBindValue" @updateConnectNameType="updateConnectNameType" v-else-if="!isEndNode()" />
  </section>
</template>
<script lang="ts" setup>
  ...
  function initApproverNodeData() {
    ...
    // 右键点击工序节点时，计算并存储parentId（上一个工序节点的nodeId）
    const prevProcessingNodeId = getPrevProcessingNodeId();
    if (prevProcessingNodeId) {
      properties.parentId = prevProcessingNodeId;
    } else {
      // 第一个工序节点，parentId设为-1
      properties.parentId = '-1';
    }
    // 保存到jnpfData
    const jnpfData: any = props.bpmn?.get('jnpfData');
    if (jnpfData) {
      jnpfData.setValue(props.element.id, { ...getJnpfData(props.element.id), parentId: properties.parentId });
    }
    console.log(props.element.id, 'parentId:', properties.parentId);
  ...
  // 获取上一个工序节点的nodeId
  function getPrevProcessingNodeId() {
    const jnpfData: any = props.bpmn?.get('jnpfData');
    if (!jnpfData || !props.element) return null;
    
    const incoming = toRaw(props.element)?.incoming || [];
    
    for (const connection of incoming) {
      const sourceId = connection.businessObject?.sourceRef?.id;
      if (!sourceId) continue;
      
      const sourceNode = jnpfData.getValue(sourceId);
      // 如果上一个节点是工序节点，返回它的nodeId
      if (sourceNode && (sourceNode.type === 'approver' || sourceNode.type === 'processing')) {
  ...
  // 判断是否是结束节点
  function isEndNode() {
    return props.element?.wnType ? NodeUtils.isEndNode({ type: props.element.wnType }) : false;
  }
  // 初始化通用节点数据
   ...
   */
  function updateGlobalFormId(formId, formName,fromData) {
    const jnpfData: any = props.bpmn.get('jnpfData');
    jnpfData.setValue('global', { formId, formName,fromData });
  }
