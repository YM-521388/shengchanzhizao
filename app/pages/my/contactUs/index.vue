	...
	export default {
		data() {
			return {
				...
				copyright: 'Copyright © 2026 绵阳凌翔机械制造有限公司出品',
				show: false
			}
		},
		onLoad() {
			...
			this.copyright = uni.getStorageSync('copyright') || 'Copyright © 2026 绵阳凌翔机械制造有限公司出品'
			setTimeout(() => {
				uni.hideLoading()
				this.show = true
			}, 800)
			...
			Jump(index) {
				switch (index) {
					...
					case 2:
						...
						// #ifndef APP-PLUS
						uni.navigateTo({
							url: '/pages/apply/externalLink/index?fullName=福建绵阳凌翔机械制造有限公司&url=' + encodeURIComponent(
								this.url)
						})
						// #endif
						break;
