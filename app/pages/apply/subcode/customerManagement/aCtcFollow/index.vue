							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        ...
                                        <u-form-item  label='跟单类型'  prop="F_FollowType">
                                            <JnpfSelect v-model="searchForm.F_FollowType"  :placeholder="$t('common.chooseText') + ' 跟单类型'"  :options=" f_FollowTypeOptions" :props="f_FollowTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='跟单内容'  prop="F_FollowContent">
										    <JnpfInput v-model="searchForm.F_FollowContent"  :placeholder="$t('common.inputText') + ' 跟单内容'"  />
                                        </u-form-item>
										<u-form-item  label='下次跟单时间'  prop="F_NextFollowTime">
											<JnpfDateRange v-model="searchForm.F_NextFollowTime" format='yyyy-MM-dd'/>
										</u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACtcFollow/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_FollowType = query.F_FollowType ? [query.F_FollowType] : null
                request({
                    url: '/api/example/ACtcFollow/List',
                    method: 'post',
                    data: query,
                }).then(res => {
