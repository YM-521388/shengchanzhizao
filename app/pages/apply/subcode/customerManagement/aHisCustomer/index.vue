							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='名称'  prop="F_CustomerName">
										    <JnpfInput v-model="searchForm.F_CustomerName"  :placeholder="$t('common.inputText') + ' 名称'"  />
                                        </u-form-item>
                                        <u-form-item  label='编号'  prop="F_CustomerCode">
										    <JnpfInput v-model="searchForm.F_CustomerCode"  :placeholder="$t('common.inputText') + ' 编号'"  />
                                        </u-form-item>
                                        <u-form-item  label='客户标签'  prop="F_CustomerTags">
                                            <JnpfSelect v-model="searchForm.F_CustomerTags"  :placeholder="$t('common.chooseText') + ' 客户标签'"  :options="f_CustomerTagsOptions" :props="f_CustomerTagsProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='客户地址-地址'  prop="tableField6eed25_F_Address">
										    <JnpfInput v-model="searchForm.tableField6eed25_F_Address"  :placeholder="$t('common.inputText') + ' 客户地址-地址'"  />
                                        </u-form-item>
                                        <u-form-item  label='客户联系人-联系人'  prop="tableFieldddabab_F_ContactName">
										    <JnpfInput v-model="searchForm.tableFieldddabab_F_ContactName"  :placeholder="$t('common.inputText') + ' 客户联系人-联系人'"  />
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
									...
									<view class="item-cell" v-for="(column,i) in columnList" :key="i">
										<template v-if="column.jnpfKey != 'table'">
											...
											<view class="item-cell-content" v-else-if="column.jnpfKey == 'switch'">
												<template v-if="column.prop==='F_IsFollow'">
													<view>{{item[column.prop]==='开'?'是':'否'}}</view>
												</template>
												<template v-if="column.prop==='F_State'">
													<view>{{item[column.prop]==='开'?'启用':'禁用'}}</view>
												</template>
											</view>
        ...
        data() {
            return {
                ...
                searchForm: {
                    ...
                    tableField6eed25_F_Address:undefined,
                    tableFieldddabab_F_ContactName:undefined,
				},
                downOption: {
                    use: true,
                    auto: false
      ...
      //               }
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACustomer/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_CustomerTags = query.F_CustomerTags ? [query.F_CustomerTags] : null
                request({
                    url: '/api/example/ACustomer/Listgh',
                    method: 'post',
                    data: query,
                }).then(res => {
