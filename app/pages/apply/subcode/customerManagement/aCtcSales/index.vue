							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        ...
                                        <u-form-item  label='支出日期'  prop="F_ExpendDate">
                                            <JnpfDateRange v-model="searchForm.F_ExpendDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='支出类别'  prop="F_ExpendType">
                                            <JnpfSelect v-model="searchForm.F_ExpendType"  :placeholder="$t('common.chooseText') + ' 支出类别'"  :options="f_ExpendTypeOptions" :props="f_ExpendTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='金额'  prop="F_Money">
										    <JnpfNumberRange v-model="searchForm.F_Money" />
                                        </u-form-item>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACtcSales/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_ExpendType = query.F_ExpendType ? [query.F_ExpendType] : null
                request({
                    url: '/api/example/ACtcSales/List',
                    method: 'post',
                    data: query,
                }).then(res => {
