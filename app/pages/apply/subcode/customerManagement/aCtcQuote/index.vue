							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        ...
                                        <u-form-item  label='报价单号'  prop="F_QuoteCode">
										    <JnpfInput v-model="searchForm.F_QuoteCode"  :placeholder="$t('common.inputText') + ' 报价单号'"  />
                                        </u-form-item>
                                        <u-form-item  label='交货日期'  prop="F_DeliveryDate">
                                            <JnpfDateRange v-model="searchForm.F_DeliveryDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='报价日期'  prop="F_QuoteDate">
                                            <JnpfDateRange v-model="searchForm.F_QuoteDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACtcQuote/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/ACtcQuote/List',
                    method: 'post',
                    data: query,
                }).then(res => {
