							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='合同'  prop="F_ContractId">
										    <JnpfInput v-model="searchForm.F_ContractId"  :placeholder="$t('common.inputText') + ' 合同'"  />
                                        </u-form-item>
                                        <u-form-item  label='类型(收/支)'  prop="F_Type">
                                            <JnpfSelect v-model="searchForm.F_Type"  :placeholder="$t('common.chooseText') + ' 类型(收/支)'"  :options="f_TypeOptions" :props="f_TypeProps"  filterable/>
                                        </u-form-item>
                                    <!--    <u-form-item  label='审核状态'  prop="F_AuditStatus">
										    <JnpfInput v-model="searchForm.F_AuditStatus"  :placeholder="$t('common.inputText') + ' 审核状态'"  />
                                        </u-form-item> -->
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACtcFinance/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/ACtcFinance/List',
                    method: 'post',
                    data: query,
                }).then(res => {
