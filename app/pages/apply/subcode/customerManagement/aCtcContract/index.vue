							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='合同编号'  prop="F_ContractCode">
										    <JnpfInput v-model="searchForm.F_ContractCode"  :placeholder="$t('common.inputText') + ' 合同编号'"  />
                                        </u-form-item>
                                        <u-form-item  label='客户'  prop="F_CustomerId">
                                            <JnpfSelect v-model="searchForm.F_CustomerId"  :placeholder="$t('common.chooseText') + ' 客户'"  :options="f_CustomerIdOptions" :props="f_CustomerIdProps"  filterable/>
                                        </u-form-item>
										<u-form-item  label='预付款'  prop="F_PrepayAmount">
										    <JnpfNumberRange v-model="searchForm.F_PrepayAmount" />
                                        </u-form-item>
                                        <u-form-item  label='交货日期'  prop="F_DeliveryDate">
                                            <JnpfDateRange v-model="searchForm.F_DeliveryDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='业务员'  prop="F_SalesmanId">
                                            <JnpfUserSelect v-model="searchForm.F_SalesmanId"  :placeholder="$t('common.chooseText') + ' 业务员'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='创建人员'  prop="F_CreatorUserId">
                                            <JnpfUserSelect v-model="searchForm.F_CreatorUserId"  :placeholder="$t('common.chooseText') + ' 创建人员'"  />
                                        </u-form-item>
                                        <u-form-item  label='创建时间'  prop="F_CreatorTime">
                                            <JnpfDateRange v-model="searchForm.F_CreatorTime" format=''/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
							...
							<u-swipe-action :show="item.show" :index="index" v-for="(item, index) in list" :key="item.id" :options="options" @click="actionClick" @open="open" :btnWidth="160" class="u-m-t-20">
								<view class="item" @click="goDetail(item.id)">
									...
									<view class="btn-container">
										<u-button v-if="$permission.hasBtnP('btn_payment',menuIds)"  type="primary" :plain="true" @click="toPage('1',item.id)">收款</u-button>
										<u-button v-if="$permission.hasBtnP('btn_expend',menuIds)" type="primary" :plain="true" @click="toPage('2',item.id)">支出</u-button>
									</view>
							    </view>
						    </u-swipe-action>
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ACtcContract/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
            ...
            upCallback(page) {
                ...
                query.F_CustomerId = query.F_CustomerId ? [query.F_CustomerId] : null
                request({
                    url: '/api/example/ACtcContract/List',
                    method: 'post',
                    data: query,
                }).then(res => {
			...
			toPage(type,id){
				uni.navigateTo({
					url:`./revenueExpend?id=${id}&type=${type}`
				})
			}
        }
    }
