							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='结算状态'  prop="F_SettleStatus">
                                            <JnpfSelect v-model="searchForm.F_SettleStatus"  :placeholder="$t('common.chooseText') + ' 结算状态'"  :options="f_SettleStatusOptions" :props="f_SettleStatusProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='外协类型'  prop="F_OutsourceType">
										    <JnpfInput v-model="searchForm.F_OutsourceType"  :placeholder="$t('common.inputText') + ' 外协类型'"  />
                                        </u-form-item>
                                        <u-form-item  label='结算人'  prop="F_SettleUserId">
                                            <JnpfUserSelect v-model="searchForm.F_SettleUserId"  :placeholder="$t('common.chooseText') + ' 结算人'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='结算时间'  prop="F_SettleTime">
                                            <JnpfDateRange v-model="searchForm.F_SettleTime" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdOutsourceAccept/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_SettleUserId = query.F_SettleUserId ? [query.F_SettleUserId] : null
                request({
                    url: '/api/example/AProdOutsourceAccept/List',
                    method: 'post',
                    data: query,
                }).then(res => {
