							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='生产人员'  prop="F_ProdUserId">
                                            <JnpfUserSelect v-model="searchForm.F_ProdUserId"  :placeholder="$t('common.chooseText') + ' 生产人员'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='良品数'  prop="F_GoodQty">
										    <JnpfNumberRange v-model="searchForm.F_GoodQty" />
                                        </u-form-item>
                                        <u-form-item  label='不良品数'  prop="F_GoodNotQty">
										    <JnpfNumberRange v-model="searchForm.F_GoodNotQty" />
                                        </u-form-item>
                                        <u-form-item  label='开始时间'  prop="F_StartTime">
                                            <JnpfDateRange v-model="searchForm.F_StartTime" format='yyyy-MM-dd HH:mm'/>
                                        </u-form-item>
                                        <u-form-item  label='结束时间'  prop="F_EndTime">
                                            <JnpfDateRange v-model="searchForm.F_EndTime" format='yyyy-MM-dd HH:mm'/>
                                        </u-form-item>
                                        <u-form-item  label='不良品项'  prop="F_CreatorUserId">
                                            <JnpfSelect v-model="searchForm.F_CreatorUserId"  :placeholder="$t('common.chooseText') + ' 不良品项'"  :options="f_CreatorUserIdOptions" :props="f_CreatorUserIdProps"  filterable/>
                                        </u-form-item>
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdReport/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
				...
				getDataInterfaceRes('2014256316289257472',query).then(res => {
					let data = res.data
					this.f_CreatorUserIdOptions = data
				});
			},
			...
			gettableField579169_F_DefectIdOptions(){
				...
				getDataInterfaceRes('2011648481097289728',query).then(res => {
					let data = res.data
					this.tableField579169_F_DefectIdOptions = data
				});
			},
            upCallback(page) {
                let query = {
                ...
                request({
                    url: '/api/example/AProdReport/List',
                    method: 'post',
                    data: query,
                }).then(res => {
