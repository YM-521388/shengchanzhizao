							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='机台名称'  prop="F_MachineName">
										    <JnpfInput v-model="searchForm.F_MachineName"  :placeholder="$t('common.inputText') + ' 机台名称'"  />
                                        </u-form-item>
                                        <u-form-item  label='机台类别'  prop="F_MachineType">
                                            <JnpfCascader v-model="searchForm.F_MachineType"  :placeholder="$t('common.chooseText') + ' 机台类别'"  :options="f_MachineTypeOptions" :props="f_MachineTypeProps"  filterable/>
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='产线'  prop="F_LineId">
                                            <JnpfSelect v-model="searchForm.F_LineId"  :placeholder="$t('common.chooseText') + ' 产线'"  :options="f_LineIdOptions" :props="f_LineIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='机台状态'  prop="F_MachineStatus">
                                            <JnpfSelect v-model="searchForm.F_MachineStatus"  :placeholder="$t('common.chooseText') + ' 机台状态'"  :options="f_MachineStatusOptions" :props="f_MachineStatusProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
...
<script>
  ...
  "children": "children"
        ...
        onLoad(e) {
			...
			this.getColumnList();
			this.getf_MachineStatusOptions();
			this.getf_MachineTypeOptions();
			this.getf_WorkshopIdOptions();
			this.getf_LineIdOptions();
			uni.hideLoading();
        },
        onUnload() {
            uni.$off('refresh')
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdMachine/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_MachineStatusOptions(){
				getDictionaryDataSelector('2011620395194650624').then(res => {
					this.f_MachineStatusOptions = res.data.list
				});
			},
			getf_MachineTypeOptions(){
				...
				getDataInterfaceRes('2011621811959238656',query).then(res => {
					let data = res.data
					this.f_MachineTypeOptions = data
				});
			},
			...
			getf_LineIdOptions(){
				getDictionaryDataSelector('2011623836151320576').then(res => {
					this.f_LineIdOptions = res.data.list
				});
			},
            upCallback(page) {
                let query = {
                ...
                request({
                    url: '/api/example/AProdMachine/List',
                    method: 'post',
                    data: query,
                }).then(res => {
