							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='生产人员'  prop="F_ProdUserIds">
                                            <JnpfUserSelect v-model="searchForm.F_ProdUserIds"  :placeholder="$t('common.chooseText') + ' 生产人员'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='质检人员'  prop="F_QcUserIds">
                                            <JnpfUserSelect v-model="searchForm.F_QcUserIds"  :placeholder="$t('common.chooseText') + ' 质检人员'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='不良品项'  prop="F_DefectIds">
                                            <JnpfSelect v-model="searchForm.F_DefectIds"  :placeholder="$t('common.chooseText') + ' 不良品项'"  :options="f_DefectIdsOptions" :props="f_DefectIdsProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='工序名'  prop="F_ProcessName">
										    <JnpfInput v-model="searchForm.F_ProcessName"  :placeholder="$t('common.inputText') + ' 工序名'"  />
                                        </u-form-item>
                                        <u-form-item  label='工序编号'  prop="F_ProcessCode">
										    <JnpfInput v-model="searchForm.F_ProcessCode"  :placeholder="$t('common.inputText') + ' 工序编号'"  />
                                        </u-form-item>
										
										
										
                                        <u-form-item  label='车间'  prop="F_WorkshopId">
											<JnpfCascader v-model="searchForm.F_WorkshopId" :placeholder="$t('common.chooseText') + ' 车间'" :options="f_WorkshopIdOptions" :props="f_WorkshopIdProps" filterable  />
                                             <!-- <JnpfCascader v-model="searchForm.F_WorkshopId"  :placeholder="$t('common.chooseText') + ' 车间'"  :options="f_WorkshopIdOptions" :props="f_WorkshopIdProps"  filterable/> -->
                                        </u-form-item>
                                        <u-form-item  label='产线'  prop="F_Line">
                                            <JnpfSelect v-model="searchForm.F_Line"  :placeholder="$t('common.chooseText') + ' 产线'"  :options="f_LineOptions" :props="f_LineProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
								...
								<view class="u-m-b-10 checkbox_box" v-if="showTop">
									...
									<view class="item-cell" v-for="(column,i) in columnList" :key="i">
										<template v-if="column.jnpfKey != 'table'">
											...
											<view class="item-cell-content" v-else-if="column.jnpfKey == 'switch'">
												<JnpfSwitch v-model="item[column.prop]" disabled />
											</view>
  ...
  "children": "children"
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdProcess/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/AProdProcess/List',
                    method: 'post',
                    data: query,
                }).then(res => {
