							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='外协工单号'  prop="F_OutsourceNo">
										    <JnpfInput v-model="searchForm.F_OutsourceNo"  :placeholder="$t('common.inputText') + ' 外协工单号'"  />
                                        </u-form-item>
                                        <u-form-item  label='供应商'  prop="F_SupplierId">
                                            <JnpfSelect v-model="searchForm.F_SupplierId"  :placeholder="$t('common.chooseText') + ' 供应商'"  :options="f_SupplierIdOptions" :props="f_SupplierIdProps"  filterable/>
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='供应商联系人电话'  prop="F_ContactPhone">
										    <JnpfInput v-model="searchForm.F_ContactPhone"  :placeholder="$t('common.inputText') + ' 供应商联系人电话'"  />
                                        </u-form-item>
                                        <u-form-item  label='供应商详细地址'  prop="F_DetailAddress">
                                            <JnpfSelect v-model="searchForm.F_DetailAddress"  :placeholder="$t('common.chooseText') + ' 供应商详细地址'"  :options="f_DetailAddressOptions" :props="f_DetailAddressProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdOutsource/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
				...
				getDataInterfaceRes('2009527238009163776',query).then(res => {
					let data = res.data
					this.f_DetailAddressOptions = data
				});
			},
            ...
            upCallback(page) {
                ...
                query.F_SupplierId = query.F_SupplierId ? [query.F_SupplierId] : null
                request({
                    url: '/api/example/AProdOutsource/List',
                    method: 'post',
                    data: query,
                }).then(res => {
