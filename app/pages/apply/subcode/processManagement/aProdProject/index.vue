							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='项目编号'  prop="F_ProjectNo">
										    <JnpfInput v-model="searchForm.F_ProjectNo"  :placeholder="$t('common.inputText') + ' 项目编号'"  />
                                        </u-form-item>
                                        <u-form-item  label='项目名称'  prop="F_ProjectName">
										    <JnpfInput v-model="searchForm.F_ProjectName"  :placeholder="$t('common.inputText') + ' 项目名称'"  />
                                        </u-form-item>
										<u-form-item  label='创建时间'  prop="F_CreatorTime">
										    <JnpfDateRange v-model="searchForm.F_CreatorTime" format=''/>
										</u-form-item>
										<u-form-item  label='修改时间'  prop="F_UpdateTime">
										    <JnpfDateRange v-model="searchForm.F_UpdateTime" format=''/>
										</u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableField751ecb_F_BomIdProps:{'label':'F_BomName','value':'F_Id'},
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdProject/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
				...
				getDataInterfaceRes('2012085692393459712',query).then(res => {
					let data = res.data
					this.tableField751ecb_F_GoodsIdOptions = data
				});
			},
			...
			gettableField751ecb_F_BomIdOptions(){
				const query = { paramList: this.jnpf.getParamList([
  {
    "defaultValue": "",
    "field": "goodsId",
    ...
    "IsMultiple": false
				...
				getDataInterfaceRes('2013188646957617152',query).then(res => {
					let data = res.data
					this.tableField751ecb_F_BomIdOptions = data
				});
			},
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/AProdProject/List',
                    method: 'post',
                    data: query,
                }).then(res => {
