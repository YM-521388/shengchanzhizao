							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='生产派工'  prop="F_ProdDispatch">
                                            <JnpfSelect v-model="searchForm.F_ProdDispatch"  :placeholder="$t('common.chooseText') + ' 生产派工'"  :options="f_ProdDispatchOptions" :props="f_ProdDispatchProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='质检派工'  prop="F_QcDispatch">
                                            <JnpfSelect v-model="searchForm.F_QcDispatch"  :placeholder="$t('common.chooseText') + ' 质检派工'"  :options="f_QcDispatchOptions" :props="f_QcDispatchProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='工序'  prop="F_ProcessId">
                                            <JnpfSelect v-model="searchForm.F_ProcessId"  :placeholder="$t('common.chooseText') + ' 工序'"  :options="f_ProcessIdOptions" :props="f_ProcessIdProps"  filterable/>
                                        </u-form-item>
										<u-form-item  label='加工单'  prop="F_ProcessingId">
										    <JnpfSelect v-model="searchForm.F_ProcessingId"  :placeholder="$t('common.chooseText') + ' 加工单'"  :options="f_ProcessingIdOptions" :props="f_ProcessingIdProps"  filterable/>
										</u-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                searchForm: {
					jnpfKeyword:'',
                    F_ProdDispatch:undefined,
                    F_QcDispatch:undefined,
        ...
        onLoad(e) {
			...
			this.getColumnList();
			this.getf_ProcessingIdOptions();
			this.getf_ProcessIdOptions();
			this.getf_TaskStatusOptions();
			this.getf_TaskTypeOptions();
			this.gettableField0bb944_F_GoodsIdOptions();
			uni.hideLoading();
        },
        onUnload() {
            uni.$off('refresh')
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AProdTask/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_ProcessingIdOptions(){
				...
				getDataInterfaceRes('2013860549426810880',query).then(res => {
					let data = res.data
					this.f_ProcessingIdOptions = data
				});
			},
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
				...
				getDataInterfaceRes('2012085692393459712',query).then(res => {
					let data = res.data
					this.tableField0bb944_F_GoodsIdOptions = data
				});
			},
            upCallback(page) {
                let query = {
                ...
                request({
                    url: '/api/example/AProdTask/List',
                    method: 'post',
                    data: query,
                }).then(res => {
			...
			jumPage(id, btnType,item){
				...
				if(btnType == 'btn_detail')url = './detail?id='+ query+'&jurisdictionType='+btnType+'&menuIds='+this.menuIds+'&idList='+idList+'&F_ProdUserByIds='+item.F_ProdUserByIds;
				uni.navigateTo({
					url: url
				})
			},
