							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='销售出库单号'  prop="F_StockOutNo">
										    <JnpfInput v-model="searchForm.F_StockOutNo"  :placeholder="$t('common.inputText') + ' 销售出库单号'"  />
                                        </u-form-item>
                                        <u-form-item  label='仓库'  prop="F_WarehouseId">
                                            <JnpfCascader v-model="searchForm.F_WarehouseId"  :placeholder="$t('common.chooseText') + ' 仓库'"  :options="f_WarehouseIdOptions" :props="f_WarehouseIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='出库类型'  prop="F_StockOutType">
                                            <JnpfSelect v-model="searchForm.F_StockOutType"  :placeholder="$t('common.chooseText') + ' 出库类型'"  :options="f_StockOutTypeOptions" :props="f_StockOutTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='出库日期'  prop="F_StockOutDate">
                                            <JnpfDateRange v-model="searchForm.F_StockOutDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='发货人'  prop="F_StockOutUserId">
                                            <JnpfUserSelect v-model="searchForm.F_StockOutUserId"  :placeholder="$t('common.chooseText') + ' 发货人'"  selectType='all' />
                                        </u-form-item>
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOutStockSo/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_WarehouseIdOptions(){
				...
				getDataInterfaceRes('2021045201115680768',query).then(res => {
					let data = res.data
					this.f_WarehouseIdOptions = data
				});
			},
				...
				const query = { paramList: this.jnpf.getParamList([
  {
    ...
    "IsMultiple": false
            ...
            upCallback(page) {
                ...
                query.F_StockOutType = query.F_StockOutType ? [query.F_StockOutType] : null
                query.F_StockOutUserId = query.F_StockOutUserId ? [query.F_StockOutUserId] : null
                request({
                    url: '/api/example/APuOutStockSo/List',
                    method: 'post',
                    data: query,
                }).then(res => {
