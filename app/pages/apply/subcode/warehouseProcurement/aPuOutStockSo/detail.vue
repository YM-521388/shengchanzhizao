		...
		<u-form :model="dataForm" ref="dataForm" :errorType="['toast']" label-position="left" :label-width="100 * 1.5" label-align="right" class="jnpf-form">
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='销售出库单号' prop="F_StockOutNo"  :label-width="100 * 1.5">
					<JnpfInput v-model="dataForm.F_StockOutNo" detailed  :maskConfig='{
  "filler": "*",
  "maskType": 1,
  "prefixType": 1,
  "prefixLimit": 0,
  ...
  "unrealMaskLength": 1
			...
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='出库类型' prop="F_StockOutType"  :label-width="100 * 1.5">
					<view class="jnpf-detail-text">{{dataForm.F_StockOutType}}</view>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='出库日期' prop="F_StockOutDate"  :label-width="100 * 1.5">
					<view class="jnpf-detail-text">{{dataForm.F_StockOutDate}}</view>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='发货人' prop="F_StockOutUserId"  :label-width="100 * 1.5">
					<view class="jnpf-detail-text">{{dataForm.F_StockOutUserId}}</view>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='客户' prop="F_CustomerId"  :label-width="100 * 1.5" class="popup-select">
				<view class="detail-text-box">
					<view class="jnpf-detail-text" >{{dataForm.F_CustomerId}}</view>
					<DisplayList v-if="Object.keys(extraData).length" :extraObj="extraData.F_CustomerId" :extraOptions='[]'></DisplayList>
				</view>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
            ...
            <view class="jnpf-table" >
				<view class="jnpf-table-item" v-for="(item,i) in dataForm.tableFielde48def" :key="i">
					<view class="jnpf-table-item-title u-flex u-row-between">
						<view class="jnpf-table-item-title-num u-line-1 u-p-r-20 u-p-l-20" @click="clickIcon('商品列表','')">{{'商品列表'}}({{i+1}})
						</view>
                    </view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='商品'prop="F_GoodsName"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableFielde48def[i].F_GoodsName}}</view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='出库数量'prop="F_Num"  :label-width="100 * 1.5">
							<JnpfInputNumber v-model="dataForm.tableFielde48def[i].F_Num" :precision='0'  detailed disabled/>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='成本单价(元)'prop="F_Price"  :label-width="100 * 1.5">
							<JnpfInputNumber v-model="dataForm.tableFielde48def[i].F_Price"  detailed disabled/>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='销售单价(元)'prop="F_SalesPrice"  :label-width="100 * 1.5">
							<JnpfInputNumber v-model="dataForm.tableFielde48def[i].F_SalesPrice"  detailed disabled/>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='类别'prop="F_Type"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableFielde48def[i].F_Type}}</view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='备注'prop="F_Description"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableFielde48def[i].F_Description}}</view>
						</u-form-item>
					</view>
                </view>
            </view>
            <view class="jnpf-table" >
				<view class="jnpf-table-item" v-for="(item,i) in dataForm.tableField67a4d7" :key="i">
					<view class="jnpf-table-item-title u-flex u-row-between">
					...
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='时间'prop="F_CreatorTime"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableField67a4d7[i].F_CreatorTime}}</view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='人员'prop="F_CreatorUserId"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableField67a4d7[i].F_CreatorUserId}}</view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='标题'prop="F_Title"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableField67a4d7[i].F_Title}}</view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='内容'prop="F_Content"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{dataForm.tableField67a4d7[i].F_Content}}</view>
						</u-form-item>
					</view>
                </view>
            </view>
    ...
    export default {
        ...
        methods: {
			...
			getF_CustomerIdExtraInfo() {
				if(!this.dataForm.F_CustomerId_id) return;
				const paramList = this.getParamList('F_CustomerId');
				const query = {
					ids: [this.dataForm.F_CustomerId_id],
				...
				getDataInterfaceDataInfoByIds('2009458830353764352 ', query).then(res => {
					...
					this.extraData.F_CustomerId = data;
				});
			},
			initCollapse() {
				setTimeout(() => {
            ...
            initData() {
                ...
                if (this.dataForm.id) {
                    request({
                        url: '/api/example/APuOutStockSo/Detail/' + this.dataForm.id,
                        method: 'get',
                    }).then(res => {
                        this.dataForm = res.data;

						this.getF_CustomerIdExtraInfo();
						this.initCollapse();
                    })
                }
            },
