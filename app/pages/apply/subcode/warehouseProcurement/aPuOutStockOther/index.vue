							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='其他出库单号'  prop="F_StockOutNo">
										    <JnpfInput v-model="searchForm.F_StockOutNo"  :placeholder="$t('common.inputText') + ' 其他出库单号'"  />
                                        </u-form-item>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOutStockOther/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuOutStockOther/List',
                    method: 'post',
                    data: query,
                }).then(res => {
