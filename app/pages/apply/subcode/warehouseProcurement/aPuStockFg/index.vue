							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='成品入库单号'  prop="F_StockInNo">
										    <JnpfInput v-model="searchForm.F_StockInNo"  :placeholder="$t('common.inputText') + ' 成品入库单号'"  />
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='工单类型'  prop="F_Type">
                                            <JnpfSelect v-model="searchForm.F_Type"  :placeholder="$t('common.chooseText') + ' 工单类型'"  :options="f_ProcessIdOptions" :props="f_ProcessIdProps"  filterable/>
                                        </u-form-item>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuStockFg/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuStockFg/List',
                    method: 'post',
                    data: query,
                }).then(res => {
