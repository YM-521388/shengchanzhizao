							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='其他入库单号'  prop="F_StockInNo">
										    <JnpfInput v-model="searchForm.F_StockInNo"  :placeholder="$t('common.inputText') + ' 其他入库单号'"  />
                                        </u-form-item>
  ...
  "children": "children"
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuStockOther/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuStockOther/List',
                    method: 'post',
                    data: query,
                }).then(res => {
