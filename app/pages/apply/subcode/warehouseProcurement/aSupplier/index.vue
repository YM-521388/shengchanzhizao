							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='供应商编号'  prop="F_SupplierNo">
										    <JnpfInput v-model="searchForm.F_SupplierNo"  :placeholder="$t('common.inputText') + ' 供应商编号'"  />
                                        </u-form-item>
                                        <u-form-item  label='供应商名称'  prop="F_SupplierName">
										    <JnpfInput v-model="searchForm.F_SupplierName"  :placeholder="$t('common.inputText') + ' 供应商名称'"  />
                                        </u-form-item>
                                        <u-form-item  label='联系人'  prop="F_ContactPerson">
										    <JnpfInput v-model="searchForm.F_ContactPerson"  :placeholder="$t('common.inputText') + ' 联系人'"  />
                                        </u-form-item>
                                        <u-form-item  label='联系人电话'  prop="F_ContactPhone">
										    <JnpfInput v-model="searchForm.F_ContactPhone"  :placeholder="$t('common.inputText') + ' 联系人电话'"  />
                                        </u-form-item>
                                        <u-form-item  label='详细地址'  prop="F_DetailAddress">
										    <JnpfInput v-model="searchForm.F_DetailAddress"  :placeholder="$t('common.inputText') + ' 详细地址'"  />
                                        </u-form-item>
                                        <u-form-item  label='供应商标签'  prop="F_SupplierTags">
                                            <JnpfSelect v-model="searchForm.F_SupplierTags"  :placeholder="$t('common.chooseText') + ' 供应商标签'"  :options="f_SupplierTagsOptions" :props="f_SupplierTagsProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/ASupplier/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_SupplierTags = query.F_SupplierTags ? [query.F_SupplierTags] : null
                request({
                    url: '/api/example/ASupplier/List',
                    method: 'post',
                    data: query,
                }).then(res => {
