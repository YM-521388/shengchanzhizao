							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='入库类型'  prop="F_StockInType">
                                            <JnpfSelect v-model="searchForm.F_StockInType"  :placeholder="$t('common.chooseText') + ' 入库类型'"  :options="f_StockInTypeOptions" :props="f_StockInTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='采购入库单号'  prop="F_StockInNo">
										    <JnpfInput v-model="searchForm.F_StockInNo"  :placeholder="$t('common.inputText') + ' 采购入库单号'"  />
                                        </u-form-item>
                                        <u-form-item  label='收货人'  prop="F_StockInUserId">
                                            <JnpfUserSelect v-model="searchForm.F_StockInUserId"  :placeholder="$t('common.chooseText') + ' 收货人'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='入库日期'  prop="F_StockInDate">
                                            <JnpfDateRange v-model="searchForm.F_StockInDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='采购单'  prop="F_pu_Orderld">
                                            <JnpfSelect v-model="searchForm.F_pu_Orderld"  :placeholder="$t('common.chooseText') + ' 采购单'"  :options="f_pu_OrderldOptions" :props="f_pu_OrderldProps"  filterable/>
                                        </u-form-item>										
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuStockPu/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_StockInType = query.F_StockInType ? [query.F_StockInType] : null
                query.F_StockInUserId = query.F_StockInUserId ? [query.F_StockInUserId] : null
				query.F_pu_Orderld = query.F_pu_Orderld ? [query.F_pu_Orderld] : null
				
                request({
                    url: '/api/example/APuStockPu/List',
                    method: 'post',
                    data: query,
                }).then(res => {
