							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='销售退货单号'  prop="F_ReturnInNo">
										    <JnpfInput v-model="searchForm.F_ReturnInNo"  :placeholder="$t('common.inputText') + ' 销售退货单号'"  />
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='入库类型'  prop="F_ReturnInType">
                                            <JnpfSelect v-model="searchForm.F_ReturnInType"  :placeholder="$t('common.chooseText') + ' 入库类型'"  :options="f_ReturnInTypeOptions" :props="f_ReturnInTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='合同单号'  prop="F_ContractId">
                                            <JnpfSelect v-model="searchForm.F_ContractId"  :placeholder="$t('common.chooseText') + ' 合同单号'"  :options="f_ContractIdOptions" :props="f_ContractIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='退货日期'  prop="F_ReturnInDate">
                                            <JnpfDateRange v-model="searchForm.F_ReturnInDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
  ...
  "children": "children"
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuReturnSo/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_ReturnInType = query.F_ReturnInType ? [query.F_ReturnInType] : null
                query.F_ContractId = query.F_ContractId ? [query.F_ContractId] : null
                request({
                    url: '/api/example/APuReturnSo/List',
                    method: 'post',
                    data: query,
                }).then(res => {
