							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='生产退料单号'  prop="F_ReturnInNo">
										    <JnpfInput v-model="searchForm.F_ReturnInNo"  :placeholder="$t('common.inputText') + ' 生产退料单号'"  />
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='加工单号'  prop="F_WorkOrderId">
                                            <JnpfSelect v-model="searchForm.F_WorkOrderId"  :placeholder="$t('common.chooseText') + ' 加工单号'"  :options="f_WorkOrderIdOptions" :props="f_WorkOrderIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='退料日期'  prop="F_ReturnInDate">
                                            <JnpfDateRange v-model="searchForm.F_ReturnInDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='工单类型'  prop="F_Type">
										    <JnpfInput v-model="searchForm.F_Type"  :placeholder="$t('common.inputText') + ' 工单类型'"  />
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuReturnPrd/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_WorkOrderIdOptions(){
				...
				getDataInterfaceRes('2014981107073814528',query).then(res => {
					let data = res.data
					this.f_WorkOrderIdOptions = data
				});
			},
            upCallback(page) {
                let query = {
                ...
                query.F_WorkOrderId = query.F_WorkOrderId ? [query.F_WorkOrderId] : null
                request({
                    url: '/api/example/APuReturnPrd/List',
                    method: 'post',
                    data: query,
                }).then(res => {
