							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='采购退货单号'  prop="F_ReturnOutNo">
										    <JnpfInput v-model="searchForm.F_ReturnOutNo"  :placeholder="$t('common.inputText') + ' 采购退货单号'"  />
                                        </u-form-item>
                                        <u-form-item  label='出库类型'  prop="F_ReturnOutType">
                                            <JnpfSelect v-model="searchForm.F_ReturnOutType"  :placeholder="$t('common.chooseText') + ' 出库类型'"  :options="f_ReturnOutTypeOptions" :props="f_ReturnOutTypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='退货日期'  prop="F_ReturnOutDate">
                                            <JnpfDateRange v-model="searchForm.F_ReturnOutDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOutReturnPu/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                query.F_ReturnOutType = query.F_ReturnOutType ? [query.F_ReturnOutType] : null
                request({
                    url: '/api/example/APuOutReturnPu/List',
                    method: 'post',
                    data: query,
                }).then(res => {
