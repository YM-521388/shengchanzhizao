							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        ...
                                        <u-form-item  label='采购人'  prop="F_UserId">
                                            <JnpfUserSelect v-model="searchForm.F_UserId"  :placeholder="$t('common.chooseText') + ' 采购人'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='交货日期'  prop="F_DeliveryDate">
                                            <JnpfDateRange v-model="searchForm.F_DeliveryDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='采购单号'  prop="F_OrderNo">
										    <JnpfInput v-model="searchForm.F_OrderNo"  :placeholder="$t('common.inputText') + ' 采购单号'"  />
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
...
<script>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOrder/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			gettableFieldf01abd_F_SupplierIdOptions(){
				const query = { paramList: this.jnpf.getParamList([]) };
				getDataInterfaceRes('2008457397298925568',query).then(res => {
					let data = res.data
					this.tableFieldf01abd_F_SupplierIdOptions = data
				});
			},
            upCallback(page) {
                let query = {
                ...
                query.F_UserId = query.F_UserId ? [query.F_UserId] : null
                request({
                    url: '/api/example/APuOrder/List',
                    method: 'post',
                    data: query,
                }).then(res => {
