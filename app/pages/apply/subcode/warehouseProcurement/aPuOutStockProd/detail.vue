...
<template>
  ...
  "unrealMaskLength": 1
			...
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='工单' prop="F_WorkOrderId"  :label-width="100 * 1.5">
					<view class="jnpf-detail-text">{{dataForm.F_WorkOrderId}}</view>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='出库套数' prop="F_Num"  :label-width="100 * 1.5">
					<JnpfInputNumber v-model="dataForm.F_Num" :precision='0'  detailed disabled/>
				</u-form-item>
			</view>
			<view class="u-p-l-20 u-p-r-20 form-item-box">
				<u-form-item label='工单类型' prop="F_Type"  :label-width="100 * 1.5">
					<JnpfInput v-model="dataForm.F_Type" detailed  :maskConfig='{
  "filler": "*",
  "maskType": 1,
  "prefixType": 1,
  "prefixLimit": 0,
				...
				<view class="jnpf-table-item" v-for="(item,i) in goodsList" :key="i">
					...
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='商品'prop="F_GoodsName"  :label-width="100 * 1.5">
							<view class="jnpf-detail-text">{{item.F_GoodsName}}<text v-if="item.F_GoodsNo">-{{item.F_GoodsNo}}</text></view>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='出库数量'prop="F_NumInfo"  :label-width="100 * 1.5">
							<JnpfInput v-model="item.F_NumInfo" disabled/>
						</u-form-item>
					</view>
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='成本单价(元)'prop="F_Price"  :label-width="100 * 1.5">
							<JnpfInputNumber v-model="item.F_Price" thousands :controls='false'  detailed disabled/>
						</u-form-item>
					</view>
<!-- 					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='入库仓库'prop="F_WarehouseId"  :label-width="100 * 1.5">
							<JnpfInput v-model="item.F_WarehouseId" disabled/>
						</u-form-item>
					</view> -->
					<view class="u-p-l-20 u-p-r-20 form-item-box">
						<u-form-item label='备注'prop="F_Description"  :label-width="100 * 1.5">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initData() {
                ...
                if (this.dataForm.id) {
                    request({
                        url: '/api/example/APuOutStockProd/Detail/' + this.dataForm.id,
                        method: 'get',
                    }).then(res => {
    ...
    "currentPage": 1,
    "pageSize": 1000,
    "F_StockOutProdId": this.dataForm.id
}
				}).then(res => {
