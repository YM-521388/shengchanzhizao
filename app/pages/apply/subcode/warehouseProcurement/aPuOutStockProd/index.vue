							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='生产出库单号'  prop="F_StockOutNo">
										    <JnpfInput v-model="searchForm.F_StockOutNo"  :placeholder="$t('common.inputText') + ' 生产出库单号'"  />
                                        </u-form-item>
                                        ...
                                        <u-form-item  label='工单号'  prop="F_WorkOrderId">
                                            <JnpfSelect v-model="searchForm.F_WorkOrderId"  :placeholder="$t('common.chooseText') + ' 工单号'"  :options="f_WorkOrderIdOptions" :props="f_WorkOrderIdProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
  ...
  "children": "children"
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOutStockProd/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuOutStockProd/List',
                    method: 'post',
                    data: query,
                }).then(res => {
