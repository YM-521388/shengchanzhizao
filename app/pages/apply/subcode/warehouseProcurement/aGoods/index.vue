							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='商品编号'  prop="F_GoodsNo">
										    <JnpfInput v-model="searchForm.F_GoodsNo"  :placeholder="$t('common.inputText') + ' 商品编号'"  />
                                        </u-form-item>
                                        <u-form-item  label='商品名称'  prop="F_GoodsName">
										    <JnpfInput v-model="searchForm.F_GoodsName"  :placeholder="$t('common.inputText') + ' 商品名称'"  />
                                        </u-form-item>
										<u-form-item  label='规格'  prop="F_Specification">
										    <JnpfInput v-model="searchForm.F_Specification"  :placeholder="$t('common.inputText') + ' 规格'"  />
                                        </u-form-item>
                                        <u-form-item  label='商品类型'  prop="F_Type">
                                            <JnpfSelect v-model="searchForm.F_Type"  :placeholder="$t('common.chooseText') + ' 商品类型'"  :options="f_TypeOptions" :props="f_TypeProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='检验规范'  prop="F_InspectRule">
										    <JnpfInput v-model="searchForm.F_InspectRule"  :placeholder="$t('common.inputText') + ' 检验规范'"  />
                                        </u-form-item>
                                        <u-form-item  label='商品编码'  prop="F_Encoding">
										    <JnpfInput v-model="searchForm.F_Encoding"  :placeholder="$t('common.inputText') + ' 商品编码'"  />
                                        </u-form-item>
                                        <u-form-item  label='类别'  prop="F_CategoryId">
                                            <JnpfCascader v-model="searchForm.F_CategoryId"  :placeholder="$t('common.chooseText') + ' 类别'"  :options="f_CategoryIdOptions" :props="f_CategoryIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='创建用户'  prop="F_CreatorUserId">
                                            <JnpfUserSelect v-model="searchForm.F_CreatorUserId"  :placeholder="$t('common.chooseText') + ' 创建用户'"  />
                                        </u-form-item>
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/AGoods/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_CategoryIdOptions(){
				...
				getDataInterfaceRes('2008414575283802112',query).then(res => {
					let data = res.data
					this.f_CategoryIdOptions = data
				});
			},
			getf_UnitOptions(){
				getDictionaryDataSelector('2008448689420505088').then(res => {
					this.f_UnitOptions = res.data.list
				});
			},
			getf_SourceOptions(){
