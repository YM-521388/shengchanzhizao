							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        ...
                                        <u-form-item  label='开票日期'  prop="F_InvoiceDate">
                                            <JnpfDateRange v-model="searchForm.F_InvoiceDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOrderInvoice/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuOrderInvoice/List',
                    method: 'post',
                    data: query,
                }).then(res => {
