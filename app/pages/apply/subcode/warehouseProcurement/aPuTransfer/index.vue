							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='调拨日期'  prop="F_TransferDate">
                                            <JnpfDateRange v-model="searchForm.F_TransferDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='发出仓库'  prop="F_FromWarehouseId">
                                            <JnpfCascader v-model="searchForm.F_FromWarehouseId"  :placeholder="$t('common.chooseText') + ' 发出仓库'"  :options="f_FromWarehouseIdOptions" :props="f_FromWarehouseIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='收入仓库'  prop="F_ToWarehouseId">
                                            <JnpfCascader v-model="searchForm.F_ToWarehouseId"  :placeholder="$t('common.chooseText') + ' 收入仓库'"  :options="f_ToWarehouseIdOptions" :props="f_ToWarehouseIdProps"  filterable/>
                                        </u-form-item>
                                        <u-form-item  label='调拨人'  prop="F_TransferUserId">
                                            <JnpfUserSelect v-model="searchForm.F_TransferUserId"  :placeholder="$t('common.chooseText') + ' 调拨人'"  selectType='all' />
                                        </u-form-item>
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuTransfer/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
			...
			getf_FromWarehouseIdOptions(){
				...
				getDataInterfaceRes('2021045201115680768',query).then(res => {
					let data = res.data
					this.f_FromWarehouseIdOptions = data
				});
			},
			getf_ToWarehouseIdOptions(){
				...
				getDataInterfaceRes('2021045201115680768',query).then(res => {
					let data = res.data
					this.f_ToWarehouseIdOptions = data
				});
			},
            upCallback(page) {
                let query = {
                ...
                query.F_TransferUserId = query.F_TransferUserId ? [query.F_TransferUserId] : null
                request({
                    url: '/api/example/APuTransfer/List',
                    method: 'post',
                    data: query,
                }).then(res => {
