							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='盘点日期'  prop="F_CheckDate">
                                            <JnpfDateRange v-model="searchForm.F_CheckDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
                                        <u-form-item  label='盘点人'  prop="F_CheckUserId">
                                            <JnpfUserSelect v-model="searchForm.F_CheckUserId"  :placeholder="$t('common.chooseText') + ' 盘点人'"  selectType='all' />
                                        </u-form-item>
                                        <u-form-item  label='盘点仓库'  prop="F_WarehouseId">
                                            <JnpfCascader v-model="searchForm.F_WarehouseId"  :placeholder="$t('common.chooseText') + ' 盘点仓库'"  :options="f_WarehouseIdOptions" :props="f_WarehouseIdProps"  filterable/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuCheck/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
