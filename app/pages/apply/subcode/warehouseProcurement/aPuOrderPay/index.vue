							...
							<view class="u-p-l-20 u-p-r-20 list">
								<scroll-view scroll-y="true" style="height: 100%;">
								    <u-form :model="searchForm" ref="searchForm" :errorType="['toast']" label-position="left" label-width="150">
                                        <u-form-item  label='采购单'  prop="F_OrderId">
										    <JnpfInput v-model="searchForm.F_OrderId"  :placeholder="$t('common.inputText') + ' 采购单'"  />
                                        </u-form-item>
                                        <u-form-item  label='付款日期'  prop="F_PayDate">
                                            <JnpfDateRange v-model="searchForm.F_PayDate" format='yyyy-MM-dd'/>
                                        </u-form-item>
								    </u-form>
								</scroll-view>
							</view>
    ...
    export default {
        ...
        methods: {
			...
			batchDelete() {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							request({
								url: '/api/example/APuOrderPay/batchRemove',
								data: _data,
								method: 'post'
							}).then(res => {
								uni.showToast({
            ...
            upCallback(page) {
                ...
                request({
                    url: '/api/example/APuOrderPay/List',
                    method: 'post',
                    data: query,
                }).then(res => {
