export class LsjFile {
	...
	addFile(file, isCallChange) {
		...
		if (file) {
			...
			// 限制文件大小
			if (file.size > units[this.prohibited.sizeUnit] * Math.abs(this.prohibited.size)) {
				this.toast(`文件大小超过${this.prohibited.size}${this.prohibited.sizeUnit}`)
				return false;
			}
	...
	_uploadHandleWX(item) {
		...
		return new Promise((resolve, reject) => {
			...
			let form = {
				...
				...this.option,
				url: this.option.url + '?parentId=' + this.option.parentId + '&fileName=' + item.file
					.name,
			...
			form['success'] = res => {
				if (res.statusCode == 200) {
					this.debug && console.log('上传完成,微信端返回不一定是字符串，根据接口返回格式判断是否需要JSON.parse：' +
						res.data)
