...
<template>
	<view class="u-numberbox">
		<view class="u-icon-minus" @click="handleClick('minus')"
		...
		<view class="u-icon-plus" @click.prevent="handleClick('plus')"
	...
	export default {
		...
		watch: {
			...
			inputVal(v1, v2) {
				...
				// if (v1 == "") return this.handleChange(v2, "change");
				let value = null;
		...
		data() {
			return {
				inputVal: null, // 输入框中的值，不能直接使用props中的value，因为应该改变props的状态
				timer: null, // 用作长按的定时器
		...
		created() {
			this.inputVal = this.valueCom || this.valueCom == 0 ? Number(this.valueCom) : null;
		},
		computed: {
			getCursorSpacing() {
				// 先将值转为px单位，再转为数值
				...
				// #endif
				// #ifdef APP-HARMONY
				return Number(this.cursorSpacing) / 2;
				// #endif
			},
			valueCom() {
				// #ifndef VUE3
		...
		methods: {
			...
			onBlur(event) {
				let val = null;
				let value = event.detail.value || null;
			...
			handleChange(value, type) {
				...
				this.$emit("input", value || value === 0 ? Number(value) : null);
				this.$emit("update:modelValue", value || value === 0 ? Number(value) : null);
				this.$emit(type, {
					// 转为Number类型
